...
def ladder_fastest_overall_rows(rows: object, mode: object) -> List[Dict[str, object]]:
    ...
    for row in filtered_public_ladder_rows(rows, mode):
        ...
        existing_rank = int(existing.get('class_rank', 0) or 0)
        candidate_rank = int(candidate.get('class_rank', 0) or 0)
        existing_updated = _leaderboard_updated_sort_value(existing)
        candidate_updated = _leaderboard_updated_sort_value(candidate)
        existing_name = str(existing.get('character_name') or '').lower()
        candidate_name = str(candidate.get('character_name') or '').lower()
        if (fastest_value, -candidate_rank, -int(bool(candidate_updated)), candidate_updated, candidate_name) < (existing_fastest, -existing_rank, -int(bool(existing_updated)), existing_updated, existing_name):
            best_by_user[identity_key] = candidate
    rows_out = list(best_by_user.values())
    rows_out.sort(key=lambda row: (float(row.get('fastest_pq_seconds') or 0.0), -int(row.get('class_rank', 0) or 0), str(row.get('character_name') or '').lower()))
...
def ladder_fastest_by_class_rows(rows: object, mode: object) -> List[Dict[str, object]]:
    ...
    for class_name in CLASS_ORDER:
        class_rows = ladder_fastest_rows_for_class(rows, mode, class_name)
        if class_rows:
...
def ladder_fastest_rows_for_class(rows: object, mode: object, target_class: object) -> List[Dict[str, object]]:
    class_name = str(target_class or '').strip()
    if class_name not in CLASS_ORDER:
        return []
    best_by_user: Dict[str, Dict[str, object]] = {}
    for row in filtered_public_ladder_rows(rows, mode):
        highest_class = str(row.get('highest_class') or '').strip()
        if highest_class != class_name:
            continue
        identity_key = _leaderboard_identity_key(row)
        if not identity_key:
            continue
        ...
        existing_name = str(existing.get('character_name') or '').lower()
        candidate_name = str(candidate.get('character_name') or '').lower()
        if (fastest_value, -candidate_level, -int(bool(candidate_updated)), candidate_updated, candidate_name) < (existing_fastest, -existing_level, -int(bool(existing_updated)), existing_updated, existing_name):
            best_by_user[identity_key] = candidate
    rows_out = list(best_by_user.values())
            ...
            with ui.column().classes('mq-arena-shell w-full gap-4'):
                ...
                if state.game_tab != 'arena':
                    ...
                    if state.game_tab == 'inventory':
                        ...
                        fastest_rows_all = ladder_fastest_overall_rows(getattr(state, 'public_ladder_rows_cache', []), current_ladder_mode)
                        ...
                        fastest_class_rows_all = ladder_fastest_rows_for_class(getattr(state, 'public_ladder_rows_cache', []), current_ladder_mode, available_fastest_class)
