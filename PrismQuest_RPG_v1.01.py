...
def merge_pq_ledger_rows(primary_rows: object, fallback_rows: object) -> List[Dict[str, object]]:
    merged: Dict[str, Dict[str, object]] = {}
    for raw_row in normalize_pq_ledger_entries(primary_rows) + normalize_pq_ledger_entries(fallback_rows):
        row = dict(raw_row)
        row_id = str(row.get('id') or '').strip()
        if not row_id:
            continue
        existing = merged.get(row_id)
        row_key = (str(row.get('updated_at') or row.get('completed_at') or ''), str(row.get('completed_at') or ''), int(row.get('attempt_index', 0) or 0))
        existing_key = (str(existing.get('updated_at') or existing.get('completed_at') or ''), str(existing.get('completed_at') or ''), int(existing.get('attempt_index', 0) or 0)) if isinstance(existing, dict) else ('', '', 0)
        if existing is None or row_key > existing_key:
            merged[row_id] = row
    rows_out = list(merged.values())
    rows_out.sort(key=lambda row: (str(row.get('completed_at') or ''), int(row.get('attempt_index', 0) or 0), str(row.get('id') or '')), reverse=True)
    return rows_out
...
def normalize_public_ladder_rows(rows: object) -> List[Dict[str, object]]:
    ...
    for raw_row in rows:
        if not isinstance(raw_row, dict):
            continue
        highest_class = normalize_player_class_name(raw_row.get('highest_class'), str(raw_row.get('highest_class') or 'Black Guard'))
        ...
        try:
            ...
            level = 1
        class_rank = class_progression_rank(highest_class)
        character_name = clean_character_name(str(raw_row.get('character_name') or 'Nameless Hero'))
...
def _public_ladder_row_identity(row: object) -> str:
    if not isinstance(row, dict):
        return ''
    mode = normalize_ladder_mode(row.get('mode'), '')
    season_id = sanitize_ladder_season_id(row.get('season_id', DEFAULT_LADDER_SEASON_ID))
    user_id = str(row.get('user_id') or '').strip()
    if user_id:
        return f'user:{user_id}:{mode}:{season_id}'
    name = clean_character_name(str(row.get('character_name') or '')).lower()
    return f'name:{name}:{mode}:{season_id}' if name else ''


def local_public_ladder_rows_for_slots(slots: object, user_id: object = '', current_global_season_id: object = DEFAULT_LADDER_SEASON_ID, global_reset_count: int = 0) -> List[Dict[str, object]]:
    rows: List[Dict[str, object]] = []
    cleaned_user_id = str(user_id or '').strip()
    for slot_index, raw_slot in enumerate(slots if isinstance(slots, list) else [], start=1):
        slot = normalize_slot_payload(raw_slot)
        snapshot = slot_leaderboard_snapshot(slot, slot_index)
        if snapshot is None:
            continue
        mode = slot_mode_from_slot_index(slot_index)
        season_id = int(current_global_season_id if slot_uses_global_ladder(mode) else sanitize_ladder_season_id(slot.get('season_id', DEFAULT_LADDER_SEASON_ID)))
        rows.append({
            'user_id': cleaned_user_id,
            'character_name': snapshot.get('character_name'),
            'level': int(snapshot.get('level', 1) or 1),
            'highest_class': snapshot.get('highest_class'),
            'class_rank': int(snapshot.get('class_rank', 0) or 0),
            'masterquest_attempts': int(slot_masterquest_attempts(slot)),
            'ladder_resets': int(slot_ladder_reset_total(slot, mode, global_reset_count)),
            'fastest_pq_seconds': snapshot.get('fastest_pq_seconds'),
            'season_id': season_id,
            'slot_index': int(slot_index),
            'mode': mode,
            'updated_at': _pq_ledger_iso_now(),
        })
    return normalize_public_ladder_rows(rows)


def merge_public_ladder_rows(primary_rows: object, fallback_rows: object) -> List[Dict[str, object]]:
    merged: Dict[str, Dict[str, object]] = {}
    for raw_row in normalize_public_ladder_rows(primary_rows) + normalize_public_ladder_rows(fallback_rows):
        row = dict(raw_row)
        identity_key = _public_ladder_row_identity(row)
        if not identity_key:
            continue
        ...
        if existing is None:
            ...
            continue
        candidate_fastest = _coerce_fastest_pq_seconds(row.get('fastest_pq_seconds'))
        existing_fastest = _coerce_fastest_pq_seconds(existing.get('fastest_pq_seconds'))
        if candidate_fastest is not None and (existing_fastest is None or candidate_fastest < existing_fastest):
            existing['fastest_pq_seconds'] = candidate_fastest
        candidate_progress = (int(row.get('class_rank', 0) or 0), int(row.get('level', 1) or 1), str(row.get('updated_at') or ''))
        existing_progress = (int(existing.get('class_rank', 0) or 0), int(existing.get('level', 1) or 1), str(existing.get('updated_at') or ''))
        if candidate_progress > existing_progress:
            preserved_fastest = existing.get('fastest_pq_seconds')
            merged[identity_key] = row
            if preserved_fastest is not None:
                merged[identity_key]['fastest_pq_seconds'] = preserved_fastest
    rows_out = list(merged.values())
    rows_out.sort(key=lambda row: (-int(row.get('class_rank', 0) or 0), -int(row.get('level', 1) or 1), str(row.get('character_name') or '').lower()))
...
def filtered_public_ladder_rows(rows: object, mode: object, current_global_season_id: object = DEFAULT_LADDER_SEASON_ID) -> List[Dict[str, object]]:
    normalized_mode = normalize_ladder_mode(mode)
    season_id = sanitize_ladder_season_id(current_global_season_id, DEFAULT_LADDER_SEASON_ID)
    if not isinstance(rows, list):
        return []
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        if normalize_ladder_mode(row.get('mode'), normalized_mode) != normalized_mode:
            continue
        if slot_uses_global_ladder(normalized_mode) and sanitize_ladder_season_id(row.get('season_id', season_id), season_id) != season_id:
            continue
...
def progression_rows_with_fastest_pq(rows: object, ledger_rows: object, mode: object, current_global_season_id: object = DEFAULT_LADDER_SEASON_ID) -> List[Dict[str, object]]:
    progression_rows = filtered_public_ladder_rows(rows, mode, current_global_season_id)
    best_by_user: Dict[str, float] = {}
    for ledger_row in filtered_pq_ledger_rows(ledger_rows, mode, current_global_season_id):
        if not bool(ledger_row.get('passed', False)):
            continue
...
class SessionState:
    def __init__(self) -> None:
        ...
        self.masterquest_result_popup_last_seen: str = ''
        ...
        self.public_ladder_cache_mode: str = ''
        self.public_pq_ledger_rows_cache: List[Dict[str, object]] = []
        self.public_pq_ledger_cache_mode: str = ''
...
def dismiss_masterquest_result_popup(self) -> None:
    ...
    self.masterquest_result_popup_last_seen = ''
...
def ensure_ladder_scene_state(self, new_visit: bool = False) -> None:
    ...
    self.refresh_global_season_state()
    current_mode = normalize_ladder_mode(getattr(self, 'ladder_mode', self.current_slot_mode() if hasattr(self, 'current_slot_mode') else 'Core'))
    cached_ladder_mode = normalize_ladder_mode(getattr(self, 'public_ladder_cache_mode', ''), '')
    cached_ledger_mode = normalize_ladder_mode(getattr(self, 'public_pq_ledger_cache_mode', ''), '')
    if new_visit or not self.public_ladder_rows_cache or (cached_ladder_mode and cached_ladder_mode != current_mode):
        self.refresh_public_ladder()
    if new_visit or not getattr(self, 'public_pq_ledger_rows_cache', []) or (cached_ledger_mode and cached_ledger_mode != current_mode):
...
def refresh_public_pq_ledger(self) -> bool:
    ...
    self.public_pq_ledger_cache_mode = mode
    local_rows = local_pq_ledger_rows_for_slots(self.slots, self.current_global_season_id)
    try:
        if not SUPABASE_ENABLED or self.supabase is None or not self.is_authenticated():
            self.public_pq_ledger_rows_cache = local_rows
            visible_rows = filtered_pq_ledger_rows(local_rows, mode, self.current_global_season_id)
            self.public_pq_ledger_status = f'Local PQ ledger holds {len(visible_rows)} etched attempt(s) for {mode}.' if visible_rows else f'No {mode} PQ attempts are etched locally yet.'
            return True
        self.refresh_global_season_state()
        ...
        response = self.supabase.table('pq_ledger_entries').select(fields).order('completed_at', desc=True).limit(5000).execute()
        remote_rows = _supabase_response_data(response)
        merged_rows = merge_pq_ledger_rows(remote_rows, local_rows)
        self.public_pq_ledger_rows_cache = merged_rows
        visible_rows = filtered_pq_ledger_rows(merged_rows, mode, self.current_global_season_id)
        if visible_rows:
            if slot_uses_global_ladder(mode):
                self.public_pq_ledger_status = f'{mode} Season {self.current_global_season_id} PQ ledger live with {len(visible_rows)} etched attempt(s).'
            else:
                self.public_pq_ledger_status = f'{mode} PQ ledger live with {len(visible_rows)} etched attempt(s).'
        else:
            if slot_uses_global_ladder(mode):
        ...
        self.public_pq_ledger_rows_cache = local_rows
        visible_rows = filtered_pq_ledger_rows(local_rows, mode, self.current_global_season_id)
        suffix = f' Showing {len(visible_rows)} local attempt(s).' if visible_rows else ''
        self.public_pq_ledger_status = f'PQ ledger unavailable until pq_ledger_entries is created: {exc}.{suffix}'
        return False
...
def refresh_public_ladder(self) -> bool:
    mode = normalize_ladder_mode(getattr(self, 'ladder_mode', self.current_slot_mode() if hasattr(self, 'current_slot_mode') else 'Core'))
    self.public_ladder_cache_mode = mode
    local_rows = local_public_ladder_rows_for_slots(self.slots, self.auth_user_id, self.current_global_season_id, self.global_ladder_reset_count)
    if not SUPABASE_ENABLED or self.supabase is None:
        self.public_ladder_rows_cache = local_rows
        visible_rows = filtered_public_ladder_rows(local_rows, mode, self.current_global_season_id)
        self.public_ladder_status = 'Global ladder unavailable in local-only mode.' if not visible_rows else f'Global ladder unavailable in local-only mode. Showing {len(visible_rows)} local contender(s) for {mode}.'
        return False
    if not self.is_authenticated():
        self.public_ladder_rows_cache = local_rows
        visible_rows = filtered_public_ladder_rows(local_rows, mode, self.current_global_season_id)
        self.public_ladder_status = "Sign in to read the registrar's global ledger." if not visible_rows else f"Sign in to read the registrar's global ledger. Showing {len(visible_rows)} local contender(s) for {mode}."
        return False
    self.refresh_global_season_state()
    try:
        ...
        try:
            response = self.supabase.table('leaderboard_entries').select(primary_fields).order('class_rank', desc=True).order('level', desc=True).order('updated_at', desc=False).execute()
        except Exception:
            response = self.supabase.table('leaderboard_entries').select(fallback_fields).order('class_rank', desc=True).order('level', desc=True).order('updated_at', desc=False).execute()
        rows = merge_public_ladder_rows(_supabase_response_data(response), local_rows)
        self.public_ladder_rows_cache = rows
        visible_rows = filtered_public_ladder_rows(rows, mode, self.current_global_season_id)
        if visible_rows:
            if slot_uses_global_ladder(mode):
                self.public_ladder_status = f"{mode} Season {self.current_global_season_id} ledger live with {len(visible_rows)} contender(s)."
            else:
                self.public_ladder_status = f'{mode} ledger live with {len(visible_rows)} contender(s). Resets are self-contained in this mode.'
        else:
            if slot_uses_global_ladder(mode):
        ...
        self.public_ladder_rows_cache = local_rows
        visible_rows = filtered_public_ladder_rows(local_rows, mode, self.current_global_season_id)
        suffix = f' Showing {len(visible_rows)} local contender(s).' if visible_rows else ''
        self.public_ladder_status = f'Global ladder unavailable until leaderboard_entries is updated for mode tabs: {exc}.{suffix}'
        return False
...
def main_page(request: Request) -> None:
    ...
    def sync_masterquest_result_dialog(force: bool = False) -> None:
        ...
        if popup_open and popup_text and str(getattr(state, 'masterquest_result_popup_last_seen', '') or '') != popup_text:
            try:
                ui.notify(popup_text, type=('negative' if popup_tone == 'danger' else 'positive'))
            except Exception:
                pass
            state.masterquest_result_popup_last_seen = popup_text
                            ...
                            if current_ladder_view == 'Guilds':
                                ...
                                if current_player_view == 'Fastest PQ by Class':
                                    with ui.card().classes('mq-card w-full p-4 md:p-5'):
                                        ...
                                        ui.label('Choose a class tab to inspect ranked clear times for that path.').classes('text-slate-300 leading-6 mb-3')
                                        ...
                                        if not fastest_class_rows_all:
                                            ui.label(f'No Prismatic Quest clear times are etched for {available_fastest_class} in this mode yet.').classes('text-slate-300 leading-6')
                                        else:
                                            with ui.card().classes('mq-panel-frame w-full px-3 py-2 mb-2'):
                                                with ui.row().classes('w-full items-center gap-3 text-[11px] uppercase tracking-[0.14em] text-slate-400 font-semibold'):
                                                    ui.label('Rank').classes('w-[58px]')
