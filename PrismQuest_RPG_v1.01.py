...
def fetch_public_profile(self, user_id: str = '', character_name: str = '') -> bool:
    ...
    if user_id and self.is_authenticated() and str(user_id).strip() == str(self.auth_user_id or '').strip():
        ...
        try:
            ...
            if user_id:
                ...
                query = query.eq('character_name', clean_character_name(character_name))
            ...
            snapshot = {}
    if not snapshot and self.player is not None and character_name and clean_character_name(character_name) == clean_character_name(self.player.name):
        snapshot = {
    ...
    self.profile_status = '' if snapshot else 'Public profile unavailable until that player has synced a public profile snapshot.'
...
def main_page(request: Request) -> None:
    ...
    with public_profile_dialog:
        ...
        def render_public_profile_dialog() -> None:
            ...
            with ui.card().classes('mq-card max-w-[980px] w-[95vw] p-6 md:p-7'):
                ...
                if not snapshot:
                    ...
                    ui.label(f'PQ Points {pq_points}  •  Ladder Resets {ladder_resets}').classes('mq-detail-text mt-2')
                    with ui.row().classes('w-full gap-4 mt-5 items-start max-[1000px]:flex-wrap'):
                        ...
                        with ui.card().classes('mq-panel-frame flex-[1.2] min-w-[360px] p-4'):
                            ...
                            for slot_key in SAVED_ITEM_SET_ORDER:
                                ...
                                for bucket in ITEM_BUCKETS:
                                    item = coerce_item(slot_items.get(bucket)) if isinstance(slot_items, dict) else None
                                    if item is None:
                                        continue
                                    shown_any = True
                                    with ui.card().classes('mq-panel-frame p-3 mt-3'):
                                        ui.label(f"{SAVED_ITEM_SET_LABELS.get(slot_key, slot_key)} • Tier {bucket}").classes('text-slate-300 text-sm tracking-[0.12em]')
                                        ui.html(f"<div class='mq-detail-text mt-2'>{hoverable_item_name_html(item, 'Empty')}</div>")
                                        ui.label(item.short_stat_text()).classes('mq-detail-text mt-2')
                            if not shown_any:
                                ui.label('No saved sets were present in the latest public snapshot.').classes('mq-detail-text mt-3')
                        ...
                        with ui.card().classes('mq-log mq-town-comm-card p-4'):
                            with ui.row().classes('w-full items-center justify-between gap-3 mb-2 max-[700px]:flex-wrap'):
                                with ui.column().classes('gap-1'):
                                    ...
                                    ui.label('Player names now open their public profile. In cloud mode, the square can also run as a shared town feed.').classes('text-slate-400 text-sm leading-6')
                                if state.is_authenticated():
                                    ui.button('Refresh Feed', on_click=lambda: (state.load_town_chat_messages(), request_render_refresh())).classes('mq-btn-secondary rounded-lg px-4 py-2 font-semibold')
                                        ...
                                        for message in state.town_communications_messages[-40:]:
                                            with ui.column().classes('w-full gap-1 mb-4'):
                                                ...
                                                with ui.row().classes('items-center gap-2 flex-wrap'):
                                                    ui.button(author_name, on_click=lambda uid=author_user_id, name=author_name: open_public_profile_dialog(uid, name)).props('flat dense no-caps').classes('text-[11px] uppercase tracking-[0.16em] text-amber-300 px-0 min-h-0')
                                                    if stamp:
                    ...
                    if state.game_tab == 'inventory':
                                            ...
                                            for member in member_rows:
                                                with ui.card().classes('mq-panel-frame p-3 mb-3'):
                                                    with ui.row().classes('w-full items-center justify-between gap-3 flex-wrap'):
                                                        ui.button(str(member.get('member_name') or 'Unknown Adventurer'), on_click=lambda uid=str(member.get('user_id') or ''), name=str(member.get('member_name') or ''): open_public_profile_dialog(uid, name)).props('flat dense no-caps').classes('text-slate-100 px-0 min-h-0 font-semibold')
                        ...
                        with ui.column().classes('w-full gap-4'):
                            with ui.card().classes('mq-card w-full p-5'):
                                ...
                                ui.label('Click a player name to open their profile. The second ladder view now ranks guilds by total PQ points earned by their members.').classes('text-slate-300 leading-7 mt-2')
                                with ui.card().classes('mq-panel-frame p-4 mt-4'):
                                            ...
                                            with ui.card().classes('mq-panel-frame p-4 mb-3'):
                                                with ui.row().classes('w-full items-center justify-between gap-3 flex-wrap'):
                                                    with ui.row().classes('items-center gap-3 flex-wrap'):
                                                        ui.label(f"#{int(row.get('rank', 0) or 0)}").classes('text-slate-400 font-semibold')
                                                        ui.button(str(row.get('character_name') or 'Nameless Hero'), on_click=lambda uid=str(row.get('user_id') or ''), name=str(row.get('character_name') or ''): open_public_profile_dialog(uid, name)).props('flat dense no-caps').classes('text-slate-100 px-0 min-h-0 font-semibold')
                                                    ui.label(f"{str(row.get('highest_class') or '')} • Level {int(row.get('level', 1) or 1)} • PQ {int(row.get('masterquest_attempts', 0) or 0)} • Resets {int(row.get('ladder_resets', 0) or 0)}").classes('mq-detail-text')
                            with ui.row().classes('gap-2 mt-4 flex-wrap'):
