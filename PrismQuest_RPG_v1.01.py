...
body {
  ...
  overflow-x: hidden;
  transform: scale(var(--mq-ui-scale));
  transform-origin: top left;
  width: calc(100% / var(--mq-ui-scale));
  min-height: calc(100vh / var(--mq-ui-scale));
}
.mq-page {
  min-height: 100vh;
  width: 100%;
        ...
        for slot_index, raw_slot in enumerate(self.slots, start=1):
            ...
            if force or self._last_public_leaderboard_hashes.get(mode) != leaderboard_hash:
                try:
                    ...
                    try:
                        self.supabase.table('leaderboard_entries').upsert(payload, on_conflict='user_id,slot_index').execute()
                    except Exception:
...
def refresh_public_ladder(self) -> bool:
    ...
    try:
        fields = 'user_id, character_name, level, highest_class, class_rank, masterquest_attempts, ladder_resets, season_id, slot_index, mode, updated_at'
        query = self.supabase.table('leaderboard_entries').select(fields).eq('mode', mode)
        if slot_uses_global_ladder(mode):
                                ...
                                if current_ladder_view == 'Players':
                                    with ui.row().classes('w-full gap-2 mt-4 flex-wrap'):
                                        for ladder_mode in LADDER_MODE_ORDER:
                                            mode_btn = ui.button(ladder_mode, on_click=lambda mode=ladder_mode: (setattr(state, 'ladder_mode', mode), state.refresh_public_ladder(), request_render_refresh())).classes('mq-btn-secondary rounded-xl px-4 py-2 font-semibold')
