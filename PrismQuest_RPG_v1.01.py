...
def open_public_profile_scene(self, user_id: str = '', character_name: str = '', return_tab: str = 'ladder') -> bool:
    label_map = {
    ...
    opened = self.fetch_public_profile(user_id=user_id, character_name=character_name)
...
def main_page(request: Request) -> None:
    ...
    def open_public_profile_dialog(user_id: str = '', character_name: str = '', return_tab: str = 'ladder') -> None:
        try:
            state.open_public_profile_scene(user_id=user_id, character_name=character_name, return_tab=return_tab)
        except Exception as exc:
                                            ...
                                            with ui.column().classes('w-full gap-1 mb-4'):
                                                ...
                                                with ui.row().classes('items-center gap-2 flex-wrap'):
                                                    ...
                                                    if author_user_id or author_name:
                                                        ui.button(icon='visibility', on_click=lambda uid=author_user_id, name=author_name: open_public_profile_dialog(uid, name, 'town')).props('flat round dense size=xs').classes('text-slate-500 !text-[10px] w-[18px] h-[18px] min-h-0 min-w-0 p-0')
                                                    if stamp:
            ...
            with ui.column().classes('mq-arena-shell w-full gap-4'):
                ...
                if state.game_tab != 'arena':
                    ...
                    if state.game_tab == 'inventory':
                                    ...
                                    if not rows:
                                        ...
                                        with ui.card().classes('mq-panel-frame w-full px-3 py-2 mb-2'):
                                            with ui.row().classes('w-full items-center gap-3 text-[11px] uppercase tracking-[0.14em] text-slate-400 font-semibold'):
                                                ...
                                                ui.label('Class').classes('min-w-[145px] flex-1')
                                        ...
                                        for row in rows:
                                            with ui.card().classes('mq-panel-frame w-full px-3 py-2 mb-1'):
                                                with ui.row().classes('w-full items-center gap-3 flex-wrap'):
                                                    ...
                                                    ui.label(str(row.get('highest_class') or '')).classes('min-w-[145px] flex-1 mq-detail-text')
                                                    ...
                                                    ui.button(icon='visibility', on_click=lambda uid=str(row.get('user_id') or ''), name=str(row.get('character_name') or ''): open_public_profile_dialog(uid, name, 'ladder')).props('flat round dense size=xs').classes('w-[18px] h-[18px] !text-[10px] text-slate-500 min-h-0 min-w-0 p-0')
                        ...
                        with ui.column().classes('w-full max-w-[1040px] mx-auto gap-4'):
                            with ui.card().classes('mq-card w-full p-6 md:p-7'):
                                ui.label('Player Profile').classes('text-3xl font-semibold text-slate-100')
                                ui.label('A cleaner dedicated scene for public profiles: basic info, equipped items, and saved slots in one place.').classes('text-slate-300 leading-7 mt-2')
                                if not snapshot:
                                    ...
                                    for slot_key in SAVED_ITEM_SET_ORDER:
                                        ...
                                        for bucket in ITEM_BUCKETS:
                                            ...
                                            if item is not None:
                                                saved_rows.append((SAVED_ITEM_SET_LABELS.get(slot_key, slot_key), int(bucket), item))
                                    ui.label(f'{character_name}  •  {highest_class}  •  Level {level}').classes('text-slate-100 text-xl mt-4')
                                    ui.label(f'PQ Points {pq_points}  •  Ladder Resets {ladder_resets}  •  Season {season_id}').classes('mq-detail-text mt-2')
                                    if updated_at:
                                        ui.label(f'Last Public Sync {updated_at}').classes('mq-detail-text mt-1')
                                    status_text = getattr(state, 'profile_status', '')
                                    if status_text:
                                        ui.label(status_text).classes('mq-detail-text mt-3')
                                    with ui.row().classes('w-full gap-4 mt-5 items-start max-[1000px]:flex-wrap'):
                                        with ui.card().classes('mq-panel-frame flex-[0.92] min-w-[260px] p-4'):
                                            ui.label('BASIC INFO').classes('mq-panel-caption')
                                            ui.label('\n'.join([
                                                f'Name: {character_name}',
                                        ...
                                        with ui.card().classes('mq-panel-frame flex-1 min-w-[290px] p-4'):
                                            ui.label('EQUIPPED ITEMS').classes('mq-panel-caption')
                                            for slot_name, label in [('weapon', 'Weapon'), ('armor', 'Armor'), ('charm', 'Charm')]:
                                                item = coerce_item(equipped.get(slot_name))
                                                with ui.card().classes('mq-panel-frame p-3 mt-3'):
                                                    ...
                                                    ui.html(f"<div class='mq-detail-text mt-2'>{hoverable_item_name_html(item, 'Empty')}</div>")
                                                    ui.label(item.short_stat_text() if item is not None else 'Empty').classes('mq-detail-text mt-2')
                                        with ui.card().classes('mq-panel-frame flex-[1.2] min-w-[330px] p-4'):
                                            ui.label('SAVED SLOTS').classes('mq-panel-caption')
                                            if not saved_rows:
                                                ui.label('No saved slots were present in the latest public snapshot.').classes('mq-detail-text mt-3')
                                            else:
                                                with ui.scroll_area().classes('w-full max-h-[460px] pr-2'):
                                                    for slot_label, bucket, item in saved_rows:
                                                        with ui.card().classes('mq-panel-frame p-3 mt-3'):
                                                            ui.label(f"{slot_label} • Tier {bucket}").classes('text-slate-300 text-sm tracking-[0.12em]')
                                                            ui.html(f"<div class='mq-detail-text mt-2'>{hoverable_item_name_html(item, 'Empty')}</div>")
