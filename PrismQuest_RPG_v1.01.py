...
CLASS_EQUIP_RULES = {
    ...
    'Monk': {'weapon': {'Staff'}, 'armor': {'Light', 'Medium'}},
    'Ninja': {'weapon': {'Dagger'}, 'armor': {'Medium'}},
    'Warlock': {'weapon': {'Staff'}, 'armor': {'Medium'}},
    'Headhunter': {'weapon': {'Axe'}, 'armor': {'Heavy'}},
...
POST_30_BASE_DAMAGE_MULTIPLIERS: Dict[int, float] = {
    30: 1.00,
    35: 0.975,
    40: 0.95,
    45: 0.925,
...
def apply_post_30_base_damage_slowdown(tables: Dict[str, Dict[int, Tuple[int, int]]]) -> Dict[str, Dict[int, Tuple[int, int]]]:
    adjusted: Dict[str, Dict[int, Tuple[int, int]]] = {}
    for subtype, curves in tables.items():
        adjusted[subtype] = {}
        for bucket, bounds in curves.items():
            low, high = bounds
            multiplier = POST_30_BASE_DAMAGE_MULTIPLIERS.get(int(bucket), 1.0)
            scaled_low = max(1, int(round(low * multiplier)))
            scaled_high = max(scaled_low, int(round(high * multiplier)))
            adjusted[subtype][bucket] = (scaled_low, scaled_high)
    return adjusted

BASE_DAMAGE_TABLES = apply_post_30_base_damage_slowdown(BASE_DAMAGE_TABLES)
    ...
    def recalculate_stats(self) -> None:
        ...
        if weapon is not None:
            ...
            if weapon.subtype == 'Axe':
                ...
                weighted_stat = (self.strength * 0.55) + (self.dexterity * 0.55)
                        ...
                        with ui.column().classes('w-full gap-4'):
                            ...
                            with ui.row().classes('w-full gap-4 items-stretch flex-wrap'):
                                with ui.card().classes('mq-card mq-glossary-section flex-1 min-w-[340px] p-5'):
                                    ...
                                    ui.label('Damage Basics').classes('text-lg font-semibold text-slate-100 mt-5 mb-3')
                                    ui.html(build_glossary_list_html([
                                        'Charms scale from INT. Caster classes use charm damage when they have enough mana.',
                                        'If a caster cannot afford the mana cost, they fall back to weapon damage instead.',
                                        'Axes mostly scale from STR.',
                                        'Daggers mostly scale from DEX.',
                                        'Staffs use both STR and DEX evenly and now get a slightly stronger stat contribution than before.',
                                        'Weapon and charm base damage come from item tier and subtype, then get boosted by Enhanced Effect and proficiency.',
                                    ])).classes('mt-3')
                                with ui.card().classes('mq-card mq-glossary-section flex-1 min-w-[340px] p-5'):
