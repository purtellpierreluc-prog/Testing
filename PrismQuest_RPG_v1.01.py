...
class SessionState:
    def __init__(self) -> None:
        ...
        self._last_slot_cache_hash: str = ''
        ...
        self._pending_cloud_sync_public: bool = False
        self._cloud_sync_delay_seconds: float = 1.2
        self._public_sync_interval_seconds: float = 15.0
        self._last_public_sync_at: float = 0.0
        cached_slots = self._stored_slot_cache()
        self.slots: List[Dict[str, object]] = cached_slots if cached_slots else ([build_default_slot_payload() for _ in range(3)] if SUPABASE_ENABLED else load_persisted_slots())
    ...
    def _stored_slot_cache(self) -> List[Dict[str, object]]:
        try:
            stored = app.storage.user.get('local_slot_cache', {})
        except Exception:
            stored = {}
        raw_slots = stored.get('slots', []) if isinstance(stored, dict) else stored
        if not isinstance(raw_slots, list):
            return []
        slots = [build_default_slot_payload() for _ in range(3)]
        found = False
        for index in range(min(3, len(raw_slots))):
            slots[index] = normalize_slot_payload(raw_slots[index])
            found = True
        return slots if found else []

    def persist_local_slot_cache(self, force: bool = False) -> None:
        slots_hash = _stable_payload_hash(self.slots)
        if not force and slots_hash == getattr(self, '_last_slot_cache_hash', ''):
            return
        try:
            app.storage.user['local_slot_cache'] = {
                'slots': [normalize_slot_payload(slot) for slot in self.slots],
                'saved_at': time.time(),
            }
            self._last_slot_cache_hash = slots_hash
        except Exception:
            pass

    def clear_local_slot_cache(self) -> None:
        try:
            app.storage.user.pop('local_slot_cache', None)
        except Exception:
            pass
        self._last_slot_cache_hash = ''

    def _queue_cloud_sync(self, slot_indexes: Optional[List[int]] = None, force: bool = False, include_public: bool = True) -> None:
        ...
        self._pending_cloud_sync_public = bool(self._pending_cloud_sync_public or include_public)
        ...
        try:
            ...
            pending_public = bool(self._pending_cloud_sync_public)
            self._pending_cloud_sync_all = False
            self._pending_cloud_sync_indexes = set()
            self._pending_cloud_sync_force = False
            self._pending_cloud_sync_public = False
            self.persist_to_cloud(slot_indexes=pending_indexes, force=pending_force, sync_public=pending_public)
            return
    ...
    async def _run_queued_cloud_sync(self) -> None:
        try:
            while True:
                ...
                delay = 0.0 if force else float(getattr(self, '_cloud_sync_delay_seconds', 1.2) or 1.2)
                if delay > 0:
                    await asyncio.sleep(delay)
                slot_indexes = None if self._pending_cloud_sync_all else sorted(self._pending_cloud_sync_indexes)
                force = bool(self._pending_cloud_sync_force)
                include_public = bool(self._pending_cloud_sync_public)
                self._pending_cloud_sync_all = False
                self._pending_cloud_sync_indexes = set()
                self._pending_cloud_sync_force = False
                self._pending_cloud_sync_public = False
                self.persist_to_cloud(slot_indexes=slot_indexes, force=force, sync_public=include_public)
                if not self._pending_cloud_sync_all and not self._pending_cloud_sync_indexes and not self._pending_cloud_sync_force and not self._pending_cloud_sync_public:
                    break
    ...
    def load_cloud_slots(self) -> bool:
        ...
        if not self.ensure_auth_session():
            self.set_auth_status('Cloud save reconnect is retrying. Your local chronicle cache is staying active.', 'warning')
            return False
        try:
            ...
            self.persist_local_slot_cache(force=True)
            self.refresh_global_season_state()
            self.apply_pending_global_season_reset()
            return True
        except Exception as exc:
            self.set_auth_status(f'Connected, but cloud save load failed: {exc}. Using the local chronicle cache until the connection settles.', 'warning')
            return False
    ...
    def persist_to_cloud(self, slot_indexes: Optional[List[int]] = None, force: bool = False, sync_public: bool = True) -> None:
        if self._cloud_sync_suspended or self.supabase is None or not self.is_authenticated():
            return
        ...
        try:
            ...
            public_sync_allowed = bool(sync_public and should_sync_public)
            if public_sync_allowed:
                now = time.monotonic()
                min_interval = 0.0 if force else float(getattr(self, '_public_sync_interval_seconds', 15.0) or 15.0)
                if force or (now - float(getattr(self, '_last_public_sync_at', 0.0) or 0.0)) >= min_interval:
                    self.sync_public_leaderboard(force=force)
                    self._last_public_sync_at = now
                else:
                    self._queue_cloud_sync(force=False, include_public=True)
        except Exception as exc:
    ...
    def persist_to_disk(self, force: bool = False) -> None:
        slots_hash = _stable_payload_hash(self.slots)
        self.persist_local_slot_cache(force=force)
        if SUPABASE_ENABLED:
            pending_indexes = [self.active_slot_index + 1] if self.active_slot_index is not None else None
            self._queue_cloud_sync(slot_indexes=pending_indexes, force=force, include_public=True)
