...
async def queue_well_encounter_async(self, refresh) -> None:
    ...
    try:
        ...
        if self.player is not None:
            if removed_item is not None and all(id(item) != id(removed_item) for item in self.player.inventory):
                            ...
                            with ui.column().classes('mq-monster-details w-full'):
                                if stage_monster is None:
                                    ...
                                    with ui.card().classes('mq-panel-frame p-4'):
                                        ...
                                        ui.label(f'ATK  {stage_monster.attack_min}-{stage_monster.attack_max}  •  SPD {stage_monster.speed}').classes('text-slate-300 text-sm mt-2')
