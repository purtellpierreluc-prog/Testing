                ...
                with ui.row().classes('mq-shell w-full items-stretch gap-6 no-wrap max-[1200px]:flex-wrap'):
                    ...
                    with ui.column().classes('w-full max-w-[470px] gap-4'):
                        with ui.card().classes('mq-side-card p-5'):
                            ...
                            if SUPABASE_ENABLED:
                                if state.is_authenticated():
                                    ...
                                    ui.label('These chronicle slots are now private to your account and sync through Supabase.').classes('text-slate-300 mt-1 leading-6')
                                    if state.auth_status:
                                        ui.html(f"<div class='mq-state-banner {state.auth_status_tone}'>{html.escape(state.auth_status)}</div>")
                                    ui.button('Sign Out', on_click=lambda: (state.sign_out(), render.refresh())).classes('w-full font-semibold tracking-wide rounded-xl py-3 mt-4 mq-btn-secondary')
                                else:
                        ...
                        with ui.card().classes('mq-side-card p-4'):
                            ui.label('Progress in the currently open slot now syncs to your account when you are signed in. Save codes still work from inside the game if you want to move a hero elsewhere.').classes('text-slate-400 leading-6')
                return
