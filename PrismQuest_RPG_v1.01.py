...
AFFIX_ROLL_BIAS_BETA = 2.1
AFFIX_ADVANTAGE_ROLLS = 2
...
def _find_title_screen_path() -> Optional[str]:
    ...
    return None
ASSET_DATA_URI_CACHE_MAX = 24

def _limited_cache_get(cache, key):
    if key not in cache:
        return None
    value = cache.pop(key)
    cache[key] = value
...
def _limited_cache_set(cache, key, value, max_size: int) -> None:
    if key in cache:
        cache.pop(key)
    cache[key] = value
...
def _image_path_to_data_uri(path: Optional[str], *, crop_alpha: bool = False) -> str:
    if path is None:
        return ''
    cache_key = (os.path.abspath(path), bool(crop_alpha))
    cached = _limited_cache_get(ASSET_DATA_URI_CACHE, cache_key)
    ...
    try:
        ...
        uri = ''
    _limited_cache_set(ASSET_DATA_URI_CACHE, cache_key, uri, ASSET_DATA_URI_CACHE_MAX)
    return uri

ASSET_DATA_URI_CACHE: OrderedDict = OrderedDict()

def _title_screen_data_uri() -> str:
    return _image_path_to_data_uri(_find_title_screen_path(), crop_alpha=True)
TITLE_SCREEN_DATA_URI = ''
...
MASTERQUEST_ESSENCE_KEYS = ['essence_one', 'essence_two', 'essence_three']
MASTERQUEST_ESSENCE_LABELS = {
    'essence_one': 'Essence I',
    'essence_two': 'Essence II',
    'essence_three': 'Essence III',
}
MASTERQUEST_VESSEL_KEYS = ['vessel_left', 'vessel_center', 'vessel_right']
MASTERQUEST_VESSEL_LABELS = {
    'vessel_left': 'Left Vessel',
    'vessel_center': 'Center Vessel',
    'vessel_right': 'Right Vessel',
}
MASTERQUEST_VESSEL_DESCRIPTIONS = {
    'vessel_left': 'A cold receptacle of black stone, waiting for the wrong star.',
    'vessel_center': 'An empty crown of glass that hums without instruction.',
    'vessel_right': 'A silent basin etched to receive exactly one true spark.',
}
...
def get_masterquest_essence_variant_data_uris() -> Dict[str, str]:
    return _build_masterquest_essence_variants()
...
def get_masterquest_prism_data_uri() -> str:
    return _masterquest_prism_data_uri()

MASTERQUEST_PRISM_DATA_URI = ''
MASTERQUEST_ESSENCE_VARIANT_DATA_URIS = {}
MASTERQUEST_ESSENCE_BLUE_DATA_URI = ''
...
def _well_monster_data_uri(index: int) -> str:
    ...
    cached = _limited_cache_get(MONSTER_DATA_URI_CACHE, key)
    if cached is not None:
        return cached
    uri = _image_path_to_data_uri(_find_well_monster_asset_path(int(index) % 5))
    _limited_cache_set(MONSTER_DATA_URI_CACHE, key, uri, 16)
    return uri
...
MONSTER_DATA_URI_CACHE: OrderedDict = OrderedDict()
...
EQUIPMENT_DATA_URI_CACHE: OrderedDict = OrderedDict()
...
def _equipment_data_uri(slot: str, subtype: str) -> str:
    ...
    cached = _limited_cache_get(EQUIPMENT_DATA_URI_CACHE, key)
    if cached is not None:
        return cached
    uri = _image_path_to_data_uri(_find_equipment_asset_path(slot, subtype))
    _limited_cache_set(EQUIPMENT_DATA_URI_CACHE, key, uri, 16)
    return uri
...
def coerce_item(item: object) -> Optional[Item]:
    ...
    if isinstance(item, dict):
        try:
            ...
            return None
    return None
def normalize_item_payload_list(items: object) -> List[Item]:
...
def _arena_monster_data_uri(monster_type: str) -> str:
    ...
    if not species:
        return ''
    cached = _limited_cache_get(MONSTER_DATA_URI_CACHE, species)
    if cached is not None:
        return cached
    uri = _image_path_to_data_uri(_find_arena_monster_asset_path(species))
    _limited_cache_set(MONSTER_DATA_URI_CACHE, species, uri, 16)
    return uri
...
WEAPON_STAT_SCALING_POWER = 1.125
SPELL_STAT_SCALING_POWER = 1.03125
ENHANCED_EFFECT_POWER = 0.625
CORE_STAT_DAMAGE_FACTOR = 0.75
...
class Item:
    ...
    def enhanced_multiplier(self) -> float:
        return 1.0 + (float(self.affix_stats.get('enhanced_effect', 0.0)) * ENHANCED_EFFECT_POWER)
...
class Player(Fighter):
    ...
    def recalculate_stats(self) -> None:
        ...
        if weapon is not None:
            ...
            weapon_stat_factor = max(0.0, (weighted_stat / 100.0) * CORE_STAT_DAMAGE_FACTOR)
            weapon_average_multiplier = 1.0 + (weapon_stat_factor * WEAPON_STAT_SCALING_POWER)
        ...
        if charm is not None:
            ...
            spell_stat_factor = max(0.0, (self.intelligence / 25.0) * CORE_STAT_DAMAGE_FACTOR)
...
def generate_monster(player_level: int, difficulty_multiplier: float = 1.0, encounter_name: Optional[str] = None, forced_level: Optional[int] = None) -> Tuple[Fighter, int]:
    ...
    late_game_toughness = monster_post25_toughness_scale(level)
    max_hp = max(10, int(round(max_hp * late_game_toughness)))
    physical_armor = max(0, int(round(physical_armor * late_game_toughness)))
    magic_resistance = max(0, int(round(magic_resistance * late_game_toughness)))
...
def _biased_affix_percentile() -> float:
    return clamp(random.betavariate(1.0, AFFIX_ROLL_BIAS_BETA), 0.0, 1.0)

def _value_from_percentile(low: float, high: float, percentile: float, *, digits: int = 3, integer: bool = False) -> float:
    percentile = clamp(percentile, 0.0, 1.0)
    value = low + ((high - low) * percentile)
    if integer:
        return int(round(value))
    return round(value, digits)

def _single_roll_value_for_stat(stat_key: str, item_level: int) -> float:
    percentile = _biased_affix_percentile()
    if stat_key == 'enhanced_effect':
        return _value_from_percentile(0.0, get_enhanced_effect_cap(item_level), percentile, digits=2)
    if stat_key in CORE_STAT_KEYS:
        return _value_from_percentile(0.0, float(get_core_stat_cap(item_level)), percentile, integer=True)
    ...
    if stat_key in {'armor_penetration', 'max_health', 'life_regen', 'life_per_kill', 'max_mana', 'mana_regen', 'mana_per_kill'}:
        return _value_from_percentile(float(low), float(high), percentile, integer=True)
    return _value_from_percentile(float(low), float(high), percentile, digits=3)

def roll_value_for_stat(stat_key: str, item_level: int, roll_mode: str = 'normal') -> float:
    first = _single_roll_value_for_stat(stat_key, item_level)
    if roll_mode not in {'advantage', 'disadvantage'}:
        return first
    best = first
    for _ in range(AFFIX_ADVANTAGE_ROLLS - 1):
        contender = _single_roll_value_for_stat(stat_key, item_level)
...
def monster_post25_toughness_scale(level: int) -> float:
    level = int(clamp(level, 1, 60))
    if level < 25:
        return 1.0
    return 1.25 + (((level - 25) / 35.0) * 1.75)
...
def marketplace_refresh_breakpoint(level: int) -> int:
    ...
    if level >= 10:
        return 10
    return 1
def generate_specific_item(item_level: int, slot: str, subtype: str, rarity: str, affix_roll_mode: str = 'normal') -> Item:
...
class SessionState:
    def __init__(self) -> None:
        ...
        self.arena_combat_log_hidden: bool = False
    ...
    def sync_active_slot(self) -> None:
        ...
        slot['vault_items'] = [item.to_dict() for item in normalize_item_payload_list(self.vault_items)]
        ...
        slot['carryover_inventory'] = [item.to_dict() for item in normalize_item_payload_list(self.shared_inventory)]
    ...
    def open_slot(self, index: int) -> None:
        ...
        self.shared_inventory = normalize_item_payload_list(self.slots[index].get('carryover_inventory', []))
        ...
        self.vault_items = normalize_item_payload_list(self.slots[index].get('vault_items', []))
...
def reset_masterquest_scene_state(self) -> None:
    ...
    self.masterquest_essence_order = list(MASTERQUEST_ESSENCE_KEYS)
    self.masterquest_container_order = list(MASTERQUEST_VESSEL_KEYS)
    ...
    self.masterquest_essence_visuals = {key: get_masterquest_essence_blue_data_uri() for key in MASTERQUEST_ESSENCE_KEYS}
...
def masterquest_visual_pool(self) -> List[str]:
    ...
    for variant_key in MASTERQUEST_ESSENCE_VARIANT_ORDER:
        uri = variants.get(variant_key, '')
        if uri:
            pool.append(uri)
    default_essence_uri = get_masterquest_essence_blue_data_uri()
    if not pool and default_essence_uri:
        pool.append(default_essence_uri)
    return pool

def assign_masterquest_essence_visuals(self) -> None:
    pool = self.masterquest_visual_pool()
    if not pool:
        self.masterquest_essence_visuals = {key: '' for key in MASTERQUEST_ESSENCE_KEYS}
        return
    if len(pool) >= len(MASTERQUEST_ESSENCE_KEYS):
        chosen = random.sample(pool, len(MASTERQUEST_ESSENCE_KEYS))
    else:
        chosen = [random.choice(pool) for _ in MASTERQUEST_ESSENCE_KEYS]
    random.shuffle(chosen)
    self.masterquest_essence_visuals = {
        essence_key: chosen[index]
        for index, essence_key in enumerate(MASTERQUEST_ESSENCE_KEYS)
    }
...
def ensure_masterquest_scene_state(self, new_visit: bool = False) -> None:
    ...
    if new_visit or not self.masterquest_attempt_active:
        ...
        shuffled_targets = list(MASTERQUEST_VESSEL_KEYS)
        random.shuffle(shuffled_targets)
        self.masterquest_solution = {essence: vessel for essence, vessel in zip(MASTERQUEST_ESSENCE_KEYS, shuffled_targets)}
        ...
        self.current_masterquest_line = random.choice([
            ...
            'Three vessels wake around the prism, each one empty in a different, threatening way.',
        ...
        self.masterquest_message = (
            'Drag a Light Essence into a vessel. Each essence belongs to exactly one receptacle, '
...
def begin_masterquest(self) -> None:
    ...
    self.open_game_tab('masterquest', 'You step into the prism chamber. Three vessels wait for stolen light.')
...
def masterquest_status_text(self) -> str:
    ...
    return (
        f"Path {path_text}  •  Solved {solved}/3  •  Blind clear odds 1 in 6  •  Entry {cost}g  •  XP Penalty -{xp_penalty_pct}%\n"
...
async def resolve_masterquest_drop(self, container_key: str, refresh) -> None:
    ...
    if essence_key in self.masterquest_matched_essences or essence_key in self.masterquest_fading_essences:
        return
    correct = bool(self.test_mode) or self.masterquest_solution.get(essence_key) == container_key
    ...
    if correct:
        ...
        self.masterquest_resolving = False
        if len(self.masterquest_matched_essences) >= len(MASTERQUEST_ESSENCE_KEYS):
...
def finish_masterquest_attempt(self, passed: bool) -> None:
    ...
    chance_text = 'guaranteed in Test Mode' if self.test_mode else '1 in 6'
    ...
    self.shared_gold = int(self.player.gold)
    carryover_inventory = [copy.deepcopy(item) for item in getattr(self.player, 'inventory', []) if coerce_item(item) is not None]
    equipped_carryover = [
        copy.deepcopy(item)
        for item in getattr(self.player, 'equipped', {}).values()
        if coerce_item(item) is not None
    ]
    self.shared_inventory = carryover_inventory + equipped_carryover
    self.shared_proficiency_levels = dict(getattr(self.player, 'proficiency_levels', empty_proficiency_levels()))
    self.shared_proficiency_progress = dict(getattr(self.player, 'proficiency_progress', empty_proficiency_progress()))
    self.player = None
    self.current_monster = None
...
def passive_regen_tick(self) -> bool:
    ...
    if self.player.hp < self.player.max_hp and passive_hp_rate > 0:
        self.life_regen_progress += passive_hp_rate / 5.0
    ...
    if self.player.max_mana > 0 and self.player.mana < self.player.max_mana and passive_mana_rate > 0:
        self.mana_regen_progress += passive_mana_rate / 5.0
...
def generate_well_item_drop(monster_level: int, player_class: str, magic_find: float) -> Item:
    item = generate_item_drop(monster_level, player_class, magic_find, affix_roll_mode='advantage')
    boosted_rarity = shift_rarity(item.rarity, 1)
    return generate_specific_item(item.level, item.slot, item.subtype, boosted_rarity, affix_roll_mode='advantage')
...
def fight(self) -> None:
    ...
    if self.player.is_alive() and not self.current_monster.is_alive():
        ...
        if random.random() < 0.88:
            item = generate_item_drop(self.current_monster.level, self.player.player_class, self.player.magic_find, affix_roll_mode=('advantage' if self.current_encounter_type == 'well' else 'disadvantage' if self.current_encounter_type == 'normal' else 'normal'))
            self.player.inventory.append(item)
            self.add_log(f'Loot found: {item.summary()}.', 'success')
        self.current_monster = None
        self.current_monster_xp = 0
...
def export_save(self) -> None:
    ...
    payload = {
        ...
        'vault_items': [item.to_dict() for item in normalize_item_payload_list(self.vault_items)],
...
def import_save(self) -> None:
    ...
    try:
        ...
        self.vault_items = normalize_item_payload_list(payload.get('vault_items', []))
...
def inn_status_text(self) -> str:
    ...
    return (
        f"{self.player.name} the {self.player.player_class}  •  Gold {self.player.gold}  •  HP {self.player.hp}/{self.player.max_hp}  •  Mana {self.player.mana}/{self.player.max_mana}\n"
        f"A room costs {cost_text}. Resting restores 35% HP (min 10), 35% Mana (min 4), and resets your Monster Chain Combo. Vault: {len(self.vault_items)}/20 items. Storage costs 5 gold; withdrawals are free."
    )
...
def store_vault_item(self, inventory_index: int) -> None:
    ...
    if self.player.gold < 5:
        ...
        return
    raw_item = self.player.inventory.pop(inventory_index)
    item = coerce_item(raw_item)
    if item is None:
        self.add_log('That item could not be stored safely.', 'warning')
        self.sync_active_slot()
        return
    self.player.gold -= 5
...
def withdraw_vault_item(self, vault_index: int) -> None:
    ...
    if vault_index < 0 or vault_index >= len(self.vault_items):
        return
    raw_item = self.vault_items.pop(vault_index)
    item = coerce_item(raw_item)
    if item is None:
        self.add_log('That vault item was corrupted and could not be withdrawn.', 'warning')
        self.sync_active_slot()
        return
    self.player.inventory.append(item)
            ...
            def render_inventory_action_bar() -> None:
                ...
                with ui.card().classes('mq-card w-full p-4').style('min-height: 132px;'):
                    ...
                    with ui.row().classes('w-full gap-2 mt-4 flex-wrap'):
                        ...
                        close_btn = ui.button('Close', on_click=lambda: (inventory_dialog.close() if popup else close_inventory_scene(), render.refresh())).classes('mq-btn-secondary rounded-lg')
            ...
            with ui.card().classes('mq-card max-w-[620px] w-[92vw] p-5'):
                ...
                if offer is None or state.player is None:
                    ...
                    with ui.row().classes('gap-3 mt-5 justify-end max-[640px]:w-full max-[640px]:flex-wrap'):
                        ...
                        ui.button('Confirm Purchase', on_click=lambda: (state.buy_marketplace_offer(state.marketplace_pending_purchase_index), setattr(state, 'marketplace_pending_purchase_index', -1), marketplace_purchase_dialog.close(), render.refresh())).classes('mq-btn-gold max-[640px]:w-full')
                    ...
                    with ui.card().classes('mq-card flex-1 min-w-[340px] p-4'):
                        ...
                        with ui.element('div').props('id=mq-inn-vault-inventory-scroll onscroll=window.mqRememberScroll&&window.mqRememberScroll("mq-inn-vault-inventory-scroll")').classes('w-full mq-pack-manifest-scroll').style('max-height: 520px; overflow-y: auto; padding-right: 6px;'):
                            if not state.player.inventory:
                                ...
                                with ui.column().classes('w-full gap-2'):
                                    for idx, raw_item in enumerate(state.player.inventory):
                                        item = coerce_item(raw_item)
                                        if item is None:
                                            continue
                    ...
                    with ui.card().classes('mq-card flex-1 min-w-[340px] p-4'):
                        ...
                        with ui.element('div').props('id=mq-inn-vault-storage-scroll onscroll=window.mqRememberScroll&&window.mqRememberScroll("mq-inn-vault-storage-scroll")').classes('w-full mq-pack-manifest-scroll').style('max-height: 520px; overflow-y: auto; padding-right: 6px;'):
                            if not state.vault_items:
                                ...
                                with ui.column().classes('w-full gap-2'):
                                    for idx, raw_item in enumerate(state.vault_items):
                                        item = coerce_item(raw_item)
                                        if item is None:
                                            continue
    ...
    with class_select_warning_dialog:
        with ui.card().classes('mq-card max-w-[520px] w-[92vw] p-5'):
            ...
            with ui.row().classes('gap-3 mt-5 justify-end max-[640px]:w-full max-[640px]:flex-wrap'):
                ...
                ui.button('Yes, Reset Hero', on_click=lambda: (class_select_warning_dialog.close(), state.back_to_class_select(), render.refresh())).classes('mq-btn-danger max-[640px]:w-full')
                ...
                with ui.row().classes('mq-shell w-full items-stretch gap-6 no-wrap max-[1200px]:flex-wrap'):
                    with ui.card().classes('mq-title-card w-full flex-1 p-4 lg:p-5'):
                        with ui.element('div').classes('mq-title-stage w-full'):
                            if _title_screen_data_uri():
                                with ui.element('div').classes('mq-title-image-wrap'):
                                    ui.html(f"<img src='{html.escape(_title_screen_data_uri(), quote=True)}' alt='MasterQuest title art' class='mq-title-image-static' loading='eager' decoding='async' draggable='false'>")
                            else:
                    ...
                    with ui.element('div').classes('mq-town-dashboard w-full'):
                        with ui.card().classes('mq-overview-card p-4 md:p-5'):
                            ...
                            with ui.element('div').classes('mt-4'):
                                with ui.element('div').classes('mq-arena-avatar-frame mq-town-avatar-frame'):
                                    if town_hero_uri:
                                        ui.html(f"<img src='{html.escape(town_hero_uri, quote=True)}' alt='{html.escape(player.player_class)} hero art' class='mq-arena-avatar-static' loading='eager' decoding='async' draggable='false'>")
                                    else:
                        ...
                        with ui.card().classes('mq-scene-card mq-town-map-card w-full p-4'):
                            ...
                            with ui.element('div').classes('mq-scene-stage mq-town-scene-stage w-full'):
                                if _town_scene_data_uri():
                                    with ui.element('div').classes('mq-scene-image-wrap mq-town-scene-image-wrap'):
                                        ui.html(f"<img src='{html.escape(_town_scene_data_uri(), quote=True)}' alt='Town map' class='mq-town-scene-image-static' loading='eager' decoding='async' draggable='false'>")
                                else:
                        ...
                        with ui.card().classes('mq-travel-card p-4 md:p-5'):
                            ...
                            with ui.element('div').classes('mq-route-grid w-full mt-4'):
                                ...
                                ui.button('Transmutation', on_click=lambda: (state.open_transmute_scene('Brass rings hum softly as Varkesh sizes up your offerings.'), render.refresh())).classes('mq-route-btn')
                                ...
                                masterquest_town_btn = ui.button('MasterQuest', on_click=lambda: (state.begin_masterquest(), render.refresh())).classes('mq-route-btn')
            ...
            with ui.column().classes('mq-arena-shell w-full gap-4'):
                    ...
                    if state.game_tab == 'inventory':
                            ...
                            with ui.row().classes('w-full items-start gap-4 max-[1250px]:flex-wrap'):
                                with ui.column().classes('flex-[0.95] min-w-[350px] gap-4'):
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        if _marketplace_scene_data_uri():
                                            with ui.element('div').classes('mq-scene-stage w-full'):
                                                with ui.element('div').classes('mq-scene-image-wrap'):
                                                    ui.html(f"<img src='{html.escape(_marketplace_scene_data_uri(), quote=True)}' alt='Marketplace scene' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                        else:
                                            with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                        ...
                        async def handle_transmute_action() -> None:
                            ...
                            if state.transmute_last_result_item is not None:
                                await state.run_transmute_reveal(render.refresh)
                        ...
                        with ui.column().classes('w-full gap-4'):
                            with ui.row().classes('w-full items-stretch gap-4 max-[1200px]:flex-wrap'):
                                ...
                                with ui.card().classes('mq-card flex-1 min-w-[360px] p-4 h-full'):
                                    ...
                                    with ui.element('div').classes('mq-scene-stage w-full'):
                                        if _transmutation_scene_data_uri():
                                            with ui.element('div').classes('mq-scene-image-wrap'):
                                                ui.html(f"<img src='{html.escape(_transmutation_scene_data_uri(), quote=True)}' alt='Transmutation sanctum' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                        else:
                                            with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                                        ...
                                        if not item_map:
                                            ...
                                            with ui.column().classes('w-full gap-4 mt-4'):
                                                ui.select(
                                                    ...
                                                    label='Offering One',
                                                    on_change=lambda e: (setattr(state, 'transmute_choice_one', e.value or ''), setattr(state, 'transmute_choice_two', ''), state.sync_transmute_selection(), render.refresh()),
                                                ).classes('w-full')
                                                ui.select(
                                                    second_options,
                                                    ...
                                                    label='Offering Two',
                                                    on_change=lambda e: (setattr(state, 'transmute_choice_two', e.value or ''), render.refresh()),
                                                ).classes('w-full')
                            ...
                            with ui.row().classes('w-full items-start gap-4 max-[1250px]:flex-wrap'):
                                with ui.column().classes('flex-[0.95] min-w-[350px] gap-4'):
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        with ui.element('div').classes('mq-scene-stage w-full'):
                                            if _well_scene_data_uri():
                                                with ui.element('div').classes('mq-scene-image-wrap'):
                                                    ui.html(f"<img src='{html.escape(_well_scene_data_uri(), quote=True)}' alt='Well of Evil scene' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                            else:
                                                with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                        ...
                        with ui.column().classes('w-full gap-4'):
                            with ui.card().classes('mq-card w-full p-5'):
                                ...
                                ui.label(f'A room costs {inn_cost_text.lower()}. Resting restores 35% HP and 35% mana while resetting your Monster Chain Combo. The Inn Vault stores up to 20 items and charges 5 gold only when you store something.').classes('text-slate-300 leading-7 mt-2')
                                with ui.card().classes('mq-panel-frame p-4 mt-4'):
                            ...
                            with ui.row().classes('w-full items-start gap-4 max-[1250px]:flex-wrap'):
                                with ui.column().classes('flex-[0.95] min-w-[350px] gap-4'):
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        with ui.element('div').classes('mq-scene-stage w-full'):
                                            if _inn_scene_data_uri():
                                                with ui.element('div').classes('mq-scene-image-wrap'):
                                                    ui.html(f"<img src='{html.escape(_inn_scene_data_uri(), quote=True)}' alt='Inn scene' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                            else:
                                                with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                                ...
                                with ui.column().classes('flex-1 min-w-[380px] gap-4'):
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        with ui.row().classes('gap-2 mt-4 flex-wrap'):
                                            rest_btn = ui.button(f'Rest ({inn_cost_text})', on_click=lambda: (state.inn_rest(), render.refresh())).classes('mq-btn-gold rounded-xl px-5 py-3 font-semibold')
                        ...
                        async def handle_masterquest_target(container_key: str) -> None:
                            await state.resolve_masterquest_drop(container_key, render.refresh)
                        ...
                        def render_masterquest_essence(essence_key: str) -> None:
                            ...
                            with ui.card().classes(' '.join(card_classes)).props('draggable=true') as card:
                                ...
                                card.on('click', lambda _e, k=essence_key: (state.select_masterquest_essence(k), render.refresh()))
                                with ui.column().classes('w-full items-center justify-between gap-3 text-center h-full'):
                                    essence_visual = state.masterquest_essence_visuals.get(essence_key, MASTERQUEST_ESSENCE_BLUE_DATA_URI)
                                    if essence_visual:
                            ...
                            with ui.element('div').classes('mq-masterquest-stage w-full'):
                                ...
                                with ui.column().classes('gap-4'):
                                    with ui.card().classes('mq-masterquest-panel-card p-4'):
                                        ui.label('Vessels').classes('mq-inv-section-title mb-3')
                                    ...
                                    with ui.card().classes('mq-masterquest-panel-card p-4'):
                                        ui.label('Blue Light Essences').classes('mq-inv-section-title mb-3')
                                    ...
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ui.label('Routes').classes('mq-inv-section-title mb-3')
                                        with ui.row().classes('gap-2 flex-wrap'):
                                            ui.button('Return to Town', on_click=lambda: (state.reset_masterquest_scene_state(), state.enter_town('You step away from the prism chamber before the rite resolves.'), render.refresh())).classes('mq-btn-gold rounded-xl px-5 py-3 font-semibold')
                                            ui.button('Return to Chronicle Slots', on_click=lambda: (state.reset_masterquest_scene_state(), state.return_to_title(), render.refresh())).classes('mq-btn-gold rounded-xl px-5 py-3 font-semibold')
                        ...
                        with ui.card().classes('mq-card w-full p-5'):
                            ...
                            with ui.row().classes('gap-2 mt-2 max-[700px]:w-full'):
                                ...
                                import_save = ui.button('Import Save Code', on_click=lambda: (state.import_save(), render.refresh())).classes('mq-btn-affirm')
                    ...
                    with ui.card().classes('mq-arena-card w-full p-5'):
                        with ui.element('div').classes('mq-player-side-layout'):
                            with ui.element('div').classes('mq-arena-avatar-frame'):
                                if hero_uri:
                                    ui.html(f"<img src='{html.escape(hero_uri, quote=True)}' alt='{html.escape(player.player_class)} hero art' class='mq-arena-avatar-static' loading='eager' decoding='async' draggable='false'>")
                                else:
                                    ui.label(player.player_class).classes('mq-arena-avatar empty')
                                ...
                                with ui.element('div').classes('mq-monster-stage mq-monster-stage-themed w-full').style(monster_theme_style(display_monster.monster_type if display_monster is not None else '')):
                                    if monster_uri:
                                        ...
                                        with ui.element('div').classes('mq-monster-image-wrap'):
                                            ui.html(f"<img src='{html.escape(monster_uri, quote=True)}' alt='{monster_alt}' class='mq-monster-image-static' loading='eager' decoding='async' draggable='false'>")
                ...
                with ui.card().classes('mq-arena-card w-full p-5'):
                    ...
                    with ui.row().classes('mq-arena-buttons w-full mt-4'):
                        ...
                        if state.fight_in_progress:
                            fight_btn.disable()
                        flee_btn = ui.button('Flee', on_click=lambda: (state.request_arena_flee(), render.refresh())).classes('mq-arena-btn secondary')
                ...
                with ui.card().classes('mq-arena-card w-full p-5'):
                    with ui.row().classes('w-full items-center justify-between gap-3 mb-3 max-[640px]:flex-wrap'):
                        ...
                        ui.button('Show Log' if state.arena_combat_log_hidden else 'Hide Log', on_click=lambda: (setattr(state, 'arena_combat_log_hidden', not state.arena_combat_log_hidden), render.refresh())).classes('mq-arena-btn secondary')
