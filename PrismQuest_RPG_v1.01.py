...
def _register_static_image(path: Optional[str], *, crop_alpha: bool = False) -> str:
    ...
    try:
        ...
        if crop_alpha and Image is not None:
            ...
            digest = hashlib.md5(f'{source_path}:{source_stat.st_mtime_ns}:{source_stat.st_size}'.encode('utf-8')).hexdigest()
            cropped_name = f"{_safe_static_stem(os.path.splitext(os.path.basename(source_path))[0], 'image')}_{digest}.png"
            cropped_path = os.path.join(GENERATED_STATIC_DIR, cropped_name)
            if not os.path.exists(cropped_path):
                with Image.open(source_path) as raw_image:
                    ...
                    image.save(cropped_path, format='PNG')
            serve_path = cropped_path
...
MASTERQUEST_PRISM_DATA_URI = _masterquest_prism_data_uri()
MASTERQUEST_ESSENCE_VARIANT_DATA_URIS = _build_masterquest_essence_variants()
MASTERQUEST_ESSENCE_BLUE_DATA_URI = MASTERQUEST_ESSENCE_VARIANT_DATA_URIS.get('blue', next(iter(MASTERQUEST_ESSENCE_VARIANT_DATA_URIS.values()), ''))
...
class Player(Fighter):
    ...
    def recalculate_stats(self) -> None:
        ...
        if weapon is not None:
            ...
            weapon_stat_factor = max(0.0, weighted_stat / 100.0) * CORE_STAT_DAMAGE_POWER
...
class SessionState:
    def __init__(self) -> None:
        ...
        self.masterquest_essence_visuals: Dict[str, str] = {key: MASTERQUEST_ESSENCE_BLUE_DATA_URI for key in MASTERQUEST_ESSENCE_KEYS}
...
def reset_masterquest_scene_state(self) -> None:
    ...
    self.masterquest_essence_visuals = {key: MASTERQUEST_ESSENCE_BLUE_DATA_URI for key in self.masterquest_essence_order}


def masterquest_visual_pool(self) -> List[str]:
    pool: List[str] = []
    for variant_key in MASTERQUEST_ESSENCE_VARIANT_ORDER:
        uri = MASTERQUEST_ESSENCE_VARIANT_DATA_URIS.get(variant_key, '')
        if uri:
            pool.append(uri)
    if not pool and MASTERQUEST_ESSENCE_BLUE_DATA_URI:
        pool.append(MASTERQUEST_ESSENCE_BLUE_DATA_URI)
    return pool
...
def generate_well_item_drop(monster_level: int, player_class: str, magic_find: float) -> Item:
    available_buckets = get_available_drop_buckets(monster_level)
    item_level = random.choice(available_buckets or [1])
    rarity = normalize_loot_rarity(shift_rarity(choose_rarity_with_magic_find(magic_find), 1))
    slot = random.choice(['weapon', 'armor', 'charm'])
    subtype = random.choice(ITEM_SUBTYPES[slot])
                ...
                with ui.row().classes('mq-shell w-full items-stretch gap-6 no-wrap max-[1200px]:flex-wrap'):
                        ...
                        with ui.element('div').classes('mq-title-stage w-full'):
                            if TITLE_SCREEN_DATA_URI:
                                with ui.element('div').classes('mq-title-image-wrap'):
                                    ui.html(f"<img src='{html.escape(TITLE_SCREEN_DATA_URI, quote=True)}' alt='MasterQuest title art' class='mq-title-image-static' loading='eager' decoding='async' draggable='false'>")
                            else:
                    ...
                    with ui.column().classes('mq-title-side-stack w-full max-w-[470px] gap-4'):
                        with ui.card().classes('mq-side-card p-5'):
                            ...
                            if SUPABASE_ENABLED:
                                if state.is_authenticated():
                                    ...
                                    ui.button('Open Chronicle Slots', on_click=lambda: (state.go_to_chronicles(), render.refresh())).classes('w-full font-semibold tracking-wide rounded-xl py-3 mt-4 mq-btn-gold')
                                    ui.button('Sign Out', on_click=lambda: (state.sign_out(), render.refresh())).classes('w-full font-semibold tracking-wide rounded-xl py-3 mt-3 mq-btn-secondary')
                                else:
                                    ...
                                    with ui.row().classes('w-full gap-3 mt-4 max-[640px]:flex-wrap'):
                                        ui.button('Create Account', on_click=lambda: (state.sign_up(), render.refresh())).classes('flex-1 font-semibold tracking-wide rounded-xl py-3 mq-btn-secondary max-[640px]:w-full')
                                        ui.button('Sign In', on_click=lambda: (state.sign_in(), render.refresh())).classes('flex-1 font-semibold tracking-wide rounded-xl py-3 mq-btn-gold max-[640px]:w-full')
                        ...
                        with ui.card().classes('mq-side-card p-4'):
                            ui.label('Title Gate').classes('text-2xl font-semibold text-slate-100')
                            ui.label('The title screen is now only the threshold. Once you authenticate, the chronicle ledger opens as its own scene before class selection and town play.').classes('text-slate-300 leading-6')
                return
                ...
                with ui.row().classes('mq-shell w-full items-stretch gap-6 no-wrap max-[1200px]:flex-wrap'):
                    ...
                    with ui.column().classes('w-full max-w-[470px] gap-4'):
                        with ui.card().classes('mq-side-card p-5'):
                            ...
                            with ui.row().classes('w-full gap-3 mt-4 max-[640px]:flex-wrap'):
                                ui.button('Back to Title', on_click=lambda: (state.return_to_title(), render.refresh())).classes('flex-1 font-semibold tracking-wide rounded-xl py-3 mq-btn-secondary max-[640px]:w-full')
                                if state.is_authenticated():
                                    ui.button('Sign Out', on_click=lambda: (state.sign_out(), render.refresh())).classes('flex-1 font-semibold tracking-wide rounded-xl py-3 mq-btn-gold max-[640px]:w-full')
                        for index in range(3):
                            ...
                            with ui.card().classes('mq-slot-card p-4'):
                                ...
                                button = ui.button(f'Open Slot {index + 1}', on_click=lambda idx=index: (state.open_slot(idx), render.refresh()))
                            ...
                            with ui.column().classes('w-full flex-[3] gap-4'):
                                ...
                                with ui.row().classes('w-full items-center justify-between gap-3 max-[800px]:flex-wrap'):
                                    with ui.row().classes('gap-2 max-[800px]:w-full'):
                                        back = ui.button('Back to Chronicle Slots' if (SUPABASE_ENABLED and state.is_authenticated()) else 'Back to Title', on_click=lambda: (state.return_to_entry_scene(), render.refresh()))
                    ...
                    with ui.card().classes('mq-town-header w-full p-5 md:p-6'):
                        with ui.row().classes('w-full items-center justify-between gap-3 max-[900px]:flex-wrap'):
                            ...
                            with ui.row().classes('gap-2 max-[900px]:w-full'):
                                ui.button('Return to Chronicle Slots', on_click=lambda: (state.go_to_chronicles(), render.refresh())).classes('font-semibold tracking-wide rounded-xl py-3 px-5 mq-btn-gold max-[900px]:w-full')
                    ...
                    with ui.element('div').classes('mq-town-dashboard w-full'):
                            ...
                            with ui.element('div').classes('mq-scene-stage mq-town-scene-stage w-full'):
                                if TOWN_SCENE_DATA_URI:
                                    with ui.element('div').classes('mq-scene-image-wrap mq-town-scene-image-wrap'):
                                        ui.html(f"<img src='{html.escape(TOWN_SCENE_DATA_URI, quote=True)}' alt='Town map' class='mq-town-scene-image-static' loading='eager' decoding='async' draggable='false'>")
                                else:
                        ...
                        with ui.card().classes('mq-travel-card p-4 md:p-5'):
                            ...
                            with ui.element('div').classes('mq-route-grid w-full mt-4'):
                                ...
                                ui.button('Bazaar', on_click=lambda: (state.open_game_tab('bazaar', 'You slip into the bazaar where adventurers barter their spoils.'), render.refresh())).classes('mq-route-btn')
            ...
            with ui.column().classes('mq-arena-shell w-full gap-4'):
                with ui.row().classes('w-full items-center justify-between gap-3 max-[900px]:flex-wrap'):
                    ...
                    with ui.row().classes('gap-2 max-[700px]:w-full'):
                        ...
                        back_to_slots = ui.button('Return to Chronicle Slots', on_click=lambda: (state.go_to_chronicles(), render.refresh()))
                    ...
                    if state.game_tab == 'inventory':
                        ...
                        with ui.column().classes('w-full gap-4'):
                            with ui.card().classes('mq-card w-full p-5'):
                                ...
                                with ui.row().classes('gap-2 mt-4 flex-wrap'):
                                    buy_tab = ui.button('Buy', on_click=lambda: (setattr(state, 'bazaar_view', 'Buy'), state.refresh_bazaar_listings(force=True), render.refresh())).classes('mq-btn-secondary rounded-xl px-5 py-3 font-semibold')
                                    sell_tab = ui.button('Sell', on_click=lambda: (setattr(state, 'bazaar_view', 'Sell'), state.refresh_bazaar_listings(force=True), render.refresh())).classes('mq-btn-secondary rounded-xl px-5 py-3 font-semibold')
                                    refresh_btn = ui.button('Refresh Board', on_click=lambda: (state.refresh_bazaar_listings(force=True), render.refresh())).classes('mq-btn-gold rounded-xl px-5 py-3 font-semibold')
                                ...
                                with ui.column().classes('flex-[0.95] min-w-[350px] gap-4'):
                                        ...
                                        if MARKETPLACE_SCENE_DATA_URI:
                                            with ui.element('div').classes('mq-scene-stage w-full'):
                                                with ui.element('div').classes('mq-scene-image-wrap'):
                                                    ui.html(f"<img src='{html.escape(MARKETPLACE_SCENE_DATA_URI, quote=True)}' alt='Bazaar scene' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                        else:
                                            with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                                    ...
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        with ui.row().classes('gap-2 flex-wrap'):
                                            ...
                                            ui.button('Return to Town', on_click=lambda: (state.enter_town('You leave the bazaar and return to the town square.'), render.refresh())).classes('mq-btn-gold rounded-xl px-5 py-3 font-semibold')
                                    ...
                                    if state.bazaar_view == 'Buy':
                                        with ui.card().classes('mq-card w-full p-4'):
                                            ...
                                            with ui.element('div').classes('mq-filter-grid w-full'):
                                                ui.select(['All tiers'] + [f'Tier {bucket}' for bucket in ITEM_BUCKETS], value=state.bazaar_tier_filter, label='Tier', on_change=lambda e: (setattr(state, 'bazaar_tier_filter', e.value), render.refresh())).classes('w-full')
                                                ui.select(ITEM_TYPE_FILTER_OPTIONS, value=state.bazaar_type_filter, label='Type', on_change=lambda e: (setattr(state, 'bazaar_type_filter', e.value), render.refresh())).classes('w-full')
                                                ui.select(ATTRIBUTE_FILTER_OPTIONS, value=state.bazaar_affix_filter, label='Affix', on_change=lambda e: (setattr(state, 'bazaar_affix_filter', e.value), render.refresh())).classes('w-full')
                                                ui.select(BAZAAR_SORT_OPTIONS, value=state.bazaar_sort, label='Sort', on_change=lambda e: (setattr(state, 'bazaar_sort', e.value), render.refresh())).classes('w-full')
                                            with ui.row().classes('gap-2 mt-4 flex-wrap'):
                                                ui.button('Reset Filters', on_click=lambda: (setattr(state, 'bazaar_tier_filter', 'All tiers'), setattr(state, 'bazaar_type_filter', 'All types'), setattr(state, 'bazaar_affix_filter', 'All attributes'), setattr(state, 'bazaar_sort', 'Newest'), render.refresh())).classes('mq-btn-gold rounded-xl px-5 py-3 font-semibold')
                                                    ...
                                                    with ui.card().classes('mq-item-card w-full p-4 mb-3').style(rarity_edge_style(item)):
                                                        with ui.row().classes('w-full items-start gap-3 max-[860px]:flex-wrap'):
                                                            ...
                                                            with ui.column().classes('items-end max-[860px]:items-start gap-2 min-w-[170px]'):
                                                                ...
                                                                ui.button('Buy Listing', on_click=lambda listing_id=listing.listing_id: (state.buy_bazaar_listing(listing_id), render.refresh())).classes('mq-btn-gold rounded-lg')
                                    else:
                                                    ...
                                                    with ui.card().classes('mq-item-card w-full p-4 mb-3').style(rarity_edge_style(item)):
                                                        with ui.row().classes('w-full items-start gap-3 max-[860px]:flex-wrap'):
                                                            ...
                                                            with ui.column().classes('items-end max-[860px]:items-start gap-2 min-w-[175px]'):
                                                                ...
                                                                list_btn = ui.button('List on Bazaar', on_click=lambda idx=inventory_index: (state.list_item_on_bazaar(idx, state.bazaar_price_input), render.refresh())).classes('mq-btn-gold rounded-lg')
                                                                if getattr(item, 'is_starter', False):
                                                                    list_btn.disable()
                                        with ui.card().classes('mq-card w-full p-4'):
                                                        ...
                                                        with ui.row().classes('w-full items-start gap-3 max-[860px]:flex-wrap'):
                                                            ...
                                                            with ui.column().classes('items-end max-[860px]:items-start gap-2 min-w-[175px]'):
                                                                ...
                                                                if listing.sold:
                                                                    ...
                                                                    ui.button('Cancel Listing', on_click=lambda listing_id=listing.listing_id: (state.cancel_bazaar_listing(listing_id), render.refresh())).classes('mq-btn-secondary rounded-lg')
                        return
                                ...
                                with ui.column().classes('flex-[0.95] min-w-[350px] gap-4'):
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        if MARKETPLACE_SCENE_DATA_URI:
                                            with ui.element('div').classes('mq-scene-stage w-full'):
                                                with ui.element('div').classes('mq-scene-image-wrap'):
                                                    ui.html(f"<img src='{html.escape(MARKETPLACE_SCENE_DATA_URI, quote=True)}' alt='Marketplace scene' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                        else:
                                            with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                                ...
                                with ui.card().classes('mq-card flex-1 min-w-[360px] p-4 h-full'):
                                    ...
                                    with ui.element('div').classes('mq-scene-stage w-full'):
                                        if TRANSMUTATION_SCENE_DATA_URI:
                                            with ui.element('div').classes('mq-scene-image-wrap'):
                                                ui.html(f"<img src='{html.escape(TRANSMUTATION_SCENE_DATA_URI, quote=True)}' alt='Transmutation sanctum' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                        else:
                                            with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                                    ...
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        with ui.element('div').classes('mq-scene-stage w-full'):
                                            if WELL_SCENE_DATA_URI:
                                                with ui.element('div').classes('mq-scene-image-wrap'):
                                                    ui.html(f"<img src='{html.escape(WELL_SCENE_DATA_URI, quote=True)}' alt='Well of Evil scene' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                            else:
                                                with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                                    ...
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        with ui.element('div').classes('mq-scene-stage w-full'):
                                            if INN_SCENE_DATA_URI:
                                                with ui.element('div').classes('mq-scene-image-wrap'):
                                                    ui.html(f"<img src='{html.escape(INN_SCENE_DATA_URI, quote=True)}' alt='Inn scene' class='mq-scene-image' loading='eager' decoding='async' draggable='false'>")
                                            else:
                                                with ui.element('div').classes('mq-scene-fallback w-full h-full'):
                            ...
                            with ui.element('div').classes('mq-masterquest-stage w-full'):
                                with ui.card().classes('mq-masterquest-prism-card p-4'):
                                    ...
                                    with ui.element('div').classes('mq-masterquest-prism-wrap'):
                                        if MASTERQUEST_PRISM_DATA_URI:
                                            ui.html(f"<img src='{html.escape(MASTERQUEST_PRISM_DATA_URI, quote=True)}' alt='Black prism' class='mq-masterquest-prism-img' loading='eager' decoding='async' draggable='false'>")
                                        else:
                                ...
                                with ui.column().classes('gap-4'):
                                    ...
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        with ui.row().classes('gap-2 flex-wrap'):
                                            ...
                                            ui.button('Return to Chronicle Slots', on_click=lambda: (state.reset_masterquest_scene_state(), state.go_to_chronicles(), render.refresh())).classes('mq-btn-gold rounded-xl px-5 py-3 font-semibold')
                        ...
                        with ui.column().classes('w-full gap-4'):
                            ...
                            with ui.card().classes('mq-card w-full p-4'):
                                ...
                                with ui.row().classes('gap-2 mt-4 flex-wrap'):
                                    ui.button('Refresh Ladder', on_click=lambda: (state.refresh_public_ladder(), render.refresh())).classes('mq-btn-secondary rounded-xl px-5 py-3 font-semibold')
...
"""
    ...
    ui.timer(0.2, maybe_refresh_for_passive_regen)
