...
def class_run_drop_debuff_thresholds(player_class: str) -> Tuple[int, int]:
    step = class_run_drop_debuff_step(player_class)
    scale = 1.075 ** step
    start_kills = int(math.ceil(300 * scale))
    cap_kills = int(math.ceil(400 * scale))
    return start_kills, max(start_kills, cap_kills)


def class_run_drop_debuff_fraction(player_class: str, run_kills: int) -> float:
    start_kills, cap_kills = class_run_drop_debuff_thresholds(player_class)
...
def class_run_drop_debuff_percent(player_class: str, run_kills: int) -> int:
    return int(round(class_run_drop_debuff_fraction(player_class, run_kills) * 100))
...
.mq-town-avatar-frame {
  border-color: rgba(118,186,255,0.18);
  background:
    radial-gradient(circle at 50% 20%, rgba(118,186,255,0.11) 0%, rgba(0,0,0,0) 28%),
    radial-gradient(circle at 50% 48%, rgba(9,14,20,0.96) 0%, rgba(5,8,12,0.985) 72%, rgba(3,4,8,1) 100%);
}
.mq-town-avatar-frame .mq-arena-avatar-static {
  filter: drop-shadow(0 28px 38px rgba(0,0,0,0.62)) drop-shadow(0 0 18px rgba(118,186,255,0.09));
}
.mq-route-grid {
  gap: 14px;
}
...
def masterquest_slot_count(player_class: Optional[str]) -> int:
    if player_class in {'Monk', 'Ninja', 'Warlock'}:
        return 4
...
def slot_highest_class(slot: Dict[str, object]) -> Optional[str]:
    ...
    progressed_candidates = [class_name for class_name in candidates if class_name not in {'Fighter', 'Mage'}]
    if progressed_candidates:
        return max(progressed_candidates, key=class_progression_rank)

    if player_class in {'Fighter', 'Mage'}:
        return player_class
    if selection_class in {'Fighter', 'Mage'}:
        return selection_class
    if 'Fighter' in unlocked_classes and 'Mage' not in unlocked_classes:
        return 'Fighter'
    if 'Mage' in unlocked_classes and 'Fighter' not in unlocked_classes:
        return 'Mage'
...
def log_status(self) -> None:
    ...
    drop_penalty_pct = class_run_drop_debuff_percent(self.player.player_class, int(getattr(self, 'current_run_kills', 0) or 0))
...
async def _finish_arena_fight_async(self, refresh) -> None:
    ...
    if self.player.is_alive() and not self.current_monster.is_alive():
        ...
        if encounter_type == 'well':
            ...
            drop_penalty = class_run_drop_debuff_fraction(self.player.player_class, self.current_run_kills)
            start_kills, cap_kills = class_run_drop_debuff_thresholds(self.player.player_class)
            ...
            if drop_penalty > 0 and self.current_run_kills in milestone_kills:
                ...
                if penalty_pct >= 100:
                    ...
                    self.add_log(f'Feeder fatigue weighs on the run: item drops are reduced by {penalty_pct}% at {self.current_run_kills} kills.', 'warning')
...
def public_profile_payload_for_slots(slots: List[Dict[str, object]], user_id: str, season_id: int, ladder_resets: int, pq_points: int) -> Optional[Dict[str, object]]:
    ...
    if entry is None:
        return None
    slot_index = max(1, int(entry.get('slot_index', 1) or 1)) - 1
    if slot_index < 0 or slot_index >= len(slots):
        return None
    slot = normalize_slot_payload(slots[slot_index])
    player_data = slot.get('player') if isinstance(slot.get('player'), dict) else None
    player = None
    if isinstance(player_data, dict):
        try:
    ...
    return {
        ...
        'saved_item_sets': slot.get('saved_item_sets', saved_item_sets_to_payload(empty_saved_item_sets())),
    }
    ...
    try:
        ...
        if isinstance(rows, list):
            for raw_row in reversed(rows):
                if not isinstance(raw_row, dict):
                    continue
                body = ' '.join(str(raw_row.get('body', '') or '').strip().split())[:220]
                if not body:
                    continue
                ...
                stamp = created_at[11:16] if len(created_at) >= 16 else ''
                messages.append({
...
def sync_public_leaderboard(self) -> None:
    ...
    try:
        ...
        if entry is None:
            self.supabase.table('leaderboard_entries').delete().eq('user_id', self.auth_user_id).execute()
            try:
                self.supabase.table('public_player_profiles').delete().eq('user_id', self.auth_user_id).execute()
            except Exception:
                pass
            return
        payload = {
        ...
        try:
            ...
            fallback_payload = {k: payload[k] for k in ('user_id', 'character_name', 'level', 'highest_class', 'class_rank', 'slot_index')}
            self.supabase.table('leaderboard_entries').upsert(fallback_payload, on_conflict='user_id').execute()
        profile_payload = public_profile_payload_for_slots(
            self.slots,
            self.auth_user_id,
            int(self.current_global_season_id),
            int(self.current_account_ladder_resets()),
            int(self.current_account_masterquest_attempts()),
        )
        if profile_payload is not None:
            try:
                self.supabase.table('public_player_profiles').upsert(profile_payload, on_conflict='user_id').execute()
            except Exception:
                pass
    except Exception as exc:
        self.public_ladder_status = f'Global ladder sync is waiting on database setup: {exc}'
...
def refresh_public_ladder(self) -> bool:
    ...
    try:
        try:
            response = (
                self.supabase.table('leaderboard_entries')
                .select('user_id, character_name, level, highest_class, class_rank, masterquest_attempts, ladder_resets, season_id, updated_at')
                .eq('season_id', int(self.current_global_season_id))
                .order('class_rank', desc=True)
                .order('level', desc=True)
                .order('updated_at', desc=False)
            ...
            response = (
                self.supabase.table('leaderboard_entries')
                .select('user_id, character_name, level, highest_class, class_rank, updated_at')
                .order('class_rank', desc=True)
                .order('level', desc=True)
                .order('updated_at', desc=False)
                .execute()
...
def fetch_public_profile(self, user_id: str = '', character_name: str = '') -> bool:
    ...
    if cleaned_user_id and self.is_authenticated() and cleaned_user_id == str(self.auth_user_id or '').strip():
        payload = public_profile_payload_for_slots(self.slots, self.auth_user_id, int(self.current_global_season_id), int(self.current_account_ladder_resets()), int(self.current_account_masterquest_attempts()))
        ...
        if not snapshot:
            try:
                query = self.supabase.table('leaderboard_entries').select('user_id, character_name, level, highest_class, class_rank, masterquest_attempts, ladder_resets, season_id, updated_at')
                if cleaned_user_id:
                    query = query.eq('user_id', cleaned_user_id)
                elif cleaned_name:
                    query = query.eq('character_name', cleaned_name)
    ...
    if not snapshot and self.player is not None and cleaned_name and cleaned_name == clean_character_name(self.player.name):
        snapshot = {
            ...
            'pq_points': int(self.current_account_masterquest_attempts()),
            'ladder_resets': int(self.current_account_ladder_resets()),
            'season_id': int(self.current_global_season_id),
...
def main_page(request: Request) -> None:
    ...
    def open_public_profile_dialog(user_id: str = '', character_name: str = '') -> None:
        try:
            state.fetch_public_profile(user_id=user_id, character_name=character_name)
        except Exception as exc:
            state.profile_snapshot = {}
            state.profile_status = f'Could not open that profile yet: {exc}'
        render_public_profile_dialog.refresh()
        public_profile_dialog.open()
            ...
            if state.screen == 'town':
                ...
                with ui.column().classes('mq-town-shell w-full gap-5'):
                    with ui.card().classes('mq-town-header w-full p-5 md:p-6'):
                        with ui.row().classes('w-full items-start justify-between gap-4 max-[900px]:flex-wrap'):
                            with ui.column().classes('gap-2 max-w-[860px]'):
                                ui.label('Chronicle in Progress').classes('mq-town-overline')
                                ui.label('Town Square').classes('text-3xl md:text-4xl font-semibold text-slate-100')
                                ui.label('Lanternlight, ledgers, and dangerous opportunities all gather here. Choose your next route, steady your build, and decide how greedy this run is about to become.').classes('mq-town-header-copy')
                    ...
                    with ui.element('div').classes('mq-town-dashboard w-full'):
                        ...
                        with ui.card().classes('mq-travel-card p-4 md:p-5'):
                            ...
                            ui.label('Choose your next risk, reset, market, or ritual.').classes('mq-town-route-note mt-2')
                        ...
                        with ui.card().classes('mq-log mq-town-comm-card p-4'):
                            with ui.row().classes('w-full items-center justify-between gap-3 mb-2 max-[700px]:flex-wrap'):
                                with ui.column().classes('gap-1'):
                                    ui.label('Town Crier & Rumors').classes('text-lg font-semibold text-slate-100')
                                    ui.label('Use the eye icon beside a speaker to open their public profile. In cloud mode, the square can also run as a shared town feed.').classes('text-slate-400 text-sm leading-6')
                                if state.is_authenticated():
                                    ui.button('Refresh Feed', on_click=lambda: (state.load_town_chat_messages(), state.load_town_coffers(), request_render_refresh())).classes('mq-btn-secondary rounded-lg px-4 py-2 font-semibold')
                                        ...
                                        for message in state.town_communications_messages[-40:]:
                                            with ui.column().classes('w-full gap-1 mb-4'):
                                                ...
                                                with ui.row().classes('items-center gap-2 flex-wrap'):
                                                    ui.label(author_name).classes('text-[11px] uppercase tracking-[0.16em] text-amber-300 font-semibold')
                                                    if author_user_id or author_name:
                                                        ui.button(icon='visibility', on_click=lambda uid=author_user_id, name=author_name: open_public_profile_dialog(uid, name)).props('flat round dense size=sm').classes('text-slate-400 min-h-0 min-w-0')
                                                    if stamp:
                                                        ui.label(f'• {stamp}').classes('text-[11px] uppercase tracking-[0.16em] text-slate-500')
                    ...
                    if state.game_tab == 'inventory':
                                                ...
                                                with ui.card().classes('mq-panel-frame p-3 mb-3'):
                                                    with ui.row().classes('w-full items-center justify-between gap-3 flex-wrap'):
                                                        with ui.row().classes('items-center gap-2'):
                                                            ...
                                                            ui.button(icon='visibility', on_click=lambda uid=str(member.get('user_id') or ''), name=str(member.get('member_name') or ''): open_public_profile_dialog(uid, name)).props('flat round dense size=sm').classes('text-slate-400 min-h-0 min-w-0')
                        ...
                        with ui.column().classes('w-full gap-4'):
                            with ui.card().classes('mq-card w-full p-5'):
                                ...
                                ui.label('The second ladder view now ranks guilds by total PQ points earned by their members. Use the eye icon on a row to inspect a player profile without cluttering the ledger.').classes('text-slate-300 leading-7 mt-2')
                                with ui.card().classes('mq-panel-frame p-4 mt-4'):
                            ...
                            with ui.row().classes('gap-2 flex-wrap'):
                                players_btn = ui.button('Players', on_click=lambda: (setattr(state, 'ladder_view', 'Players'), request_render_refresh())).classes('mq-btn-secondary rounded-xl px-5 py-3 font-semibold')
                                guilds_btn = ui.button('Guilds', on_click=lambda: (setattr(state, 'ladder_view', 'Guilds'), request_render_refresh())).classes('mq-btn-secondary rounded-xl px-5 py-3 font-semibold')
                            ...
                            if current_ladder_view == 'Guilds':
                                with ui.card().classes('mq-card w-full p-4'):
                                    ...
                                    guild_rows = getattr(state, 'guild_leaderboard_rows', []) if isinstance(getattr(state, 'guild_leaderboard_rows', []), list) else []
                                    if not guild_rows:
                                        ...
                                        for row in guild_rows:
                                            with ui.card().classes('mq-panel-frame p-4 mb-3'):
                                                with ui.row().classes('w-full items-center justify-between gap-3 flex-wrap'):
                                                    ui.label(f"#{int(row.get('rank', 0) or 0)} • {str(row.get('guild_name') or 'Unnamed Guild')}").classes('text-slate-100 font-semibold')
                                                    ui.label(f"PQ {int(row.get('pq_points', 0) or 0)} • Members {int(row.get('member_count', 0) or 0)} • Leader {str(row.get('leader_name') or 'Unknown Leader')}").classes('mq-detail-text')
                            else:
                                with ui.card().classes('mq-card w-full p-4'):
                                    ui.label('Global Progression Ladder').classes('text-xl font-semibold text-slate-100 mb-3')
                                    rows = getattr(state, 'public_ladder_rows_cache', []) if isinstance(getattr(state, 'public_ladder_rows_cache', []), list) else []
                                    if not rows:
                                        ...
                                        with ui.card().classes('mq-panel-frame p-3 mb-2'):
                                            with ui.row().classes('w-full items-center gap-3 text-[11px] uppercase tracking-[0.14em] text-slate-400 font-semibold'):
                                                ui.label('Rank').classes('w-[64px]')
                                                ui.label('Character').classes('min-w-[180px] flex-[1.1]')
                                                ui.label('Class').classes('min-w-[150px] flex-1')
                                                ui.label('Level').classes('w-[70px]')
                                                ui.label('PQ').classes('w-[70px]')
                                                ui.label('Resets').classes('w-[84px]')
                                                ui.label('').classes('w-[42px]')
                                        for row in rows:
                                            with ui.card().classes('mq-panel-frame p-3 mb-2'):
                                                with ui.row().classes('w-full items-center gap-3 flex-wrap'):
                                                    ui.label(f"#{int(row.get('rank', 0) or 0)}").classes('w-[64px] text-slate-400 font-semibold')
                                                    ui.label(str(row.get('character_name') or 'Nameless Hero')).classes('min-w-[180px] flex-[1.1] text-slate-100 font-semibold')
                                                    ui.label(str(row.get('highest_class') or '')).classes('min-w-[150px] flex-1 mq-detail-text')
                                                    ui.label(str(int(row.get('level', 1) or 1))).classes('w-[70px] mq-detail-text')
                                                    ui.label(str(int(row.get('masterquest_attempts', 0) or 0))).classes('w-[70px] mq-detail-text')
                                                    ui.label(str(int(row.get('ladder_resets', 0) or 0))).classes('w-[84px] mq-detail-text')
                                                    ui.button(icon='visibility', on_click=lambda uid=str(row.get('user_id') or ''), name=str(row.get('character_name') or ''): open_public_profile_dialog(uid, name)).props('flat round dense size=sm').classes('w-[42px] text-slate-400')
                            with ui.row().classes('gap-2 mt-4 flex-wrap'):
