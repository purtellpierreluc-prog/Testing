...
.mq-transmute-reveal-active::before {
  animation: mqTransmuteSweep 2.9s ease-in-out infinite;
}
...
@keyframes mqTransmuteSweep {
  0% { transform: translateX(-58%) rotate(7deg); opacity: 0.16; }
  18% { opacity: 0.34; }
  52% { opacity: 0.42; }
  100% { transform: translateX(58%) rotate(7deg); opacity: 0.12; }
}
.q-tab-panels,
.q-tab-panel,
.q-table tbody td,
...
TRANSMUTE_TIER_SORT_OPTIONS = ['Tier (High-Low)', 'Tier (Low-High)']
TRANSMUTE_RARITY_SORT_OPTIONS = ['Rarity (High-Low)', 'Rarity (Low-High)']
...
class SessionState:
    def __init__(self) -> None:
        ...
        self.transmute_tier_sort: str = 'Tier (High-Low)'
        self.transmute_rarity_sort: str = 'Rarity (High-Low)'
    ...
    def _apply_runtime_ladder_reset_state(self, notice: str, tone: str = 'warning') -> None:
        ...
        self.transmute_tier_sort = 'Tier (High-Low)'
        self.transmute_rarity_sort = 'Rarity (High-Low)'
...
def transmute_sort_settings(self) -> Tuple[str, str]:
    tier_sort = str(getattr(self, 'transmute_tier_sort', 'Tier (High-Low)') or 'Tier (High-Low)')
    rarity_sort = str(getattr(self, 'transmute_rarity_sort', 'Rarity (High-Low)') or 'Rarity (High-Low)')
    if tier_sort not in TRANSMUTE_TIER_SORT_OPTIONS:
        tier_sort = 'Tier (High-Low)'
    if rarity_sort not in TRANSMUTE_RARITY_SORT_OPTIONS:
        rarity_sort = 'Rarity (High-Low)'
    return tier_sort, rarity_sort
def transmute_sort_key(self, source: str, key: object, item: Item) -> Tuple[object, ...]:
    tier_sort, rarity_sort = self.transmute_sort_settings()
    ...
    try:
        ...
        rarity_rank = -1
    tier_key = -tier_value if tier_sort == 'Tier (High-Low)' else tier_value
    rarity_key = -rarity_rank if rarity_sort == 'Rarity (High-Low)' else rarity_rank
    ...
    try:
        ...
        numeric_key = 0
    return (tier_key, rarity_key, slot_key, subtype_key, name_key, source_key, numeric_key)
...
async def run_transmute_reveal(self, refresh) -> None:
    ...
    _refresh()
    await asyncio.sleep(0.42)
    for stage, pause in ((1, 0.34), (2, 0.32), (3, 0.26)):
        self.transmute_reveal_stage = stage
        _refresh()
        await asyncio.sleep(pause)
    self.transmute_reveal_stage = 4
    _refresh()
...
SessionState.transmute_sort_settings = transmute_sort_settings
                        ...
                        with ui.element('div').classes('mq-title-stage w-full'):
                            ...
                            if title_art_url:
                                with ui.element('div').classes('mq-title-image-wrap'):
                                    ui.html(
                                        f"<div class='mq-title-image-fit' style='--mq-title-art-w:{max(title_art_width, 1)}; --mq-title-art-h:{max(title_art_height, 1)};'><img src='{html.escape(title_art_url, quote=True)}' alt='MasterQuest title art' class='mq-title-image-static' loading='eager' decoding='async' draggable='false'></div>"
                                    )
                            else:
                ...
                with ui.column().classes('mq-town-shell w-full gap-5'):
                    ...
                    with ui.element('div').classes('mq-town-dashboard w-full'):
                        ...
                        with ui.card().classes('mq-log mq-town-comm-card p-4'):
                            ui.label('Town Crier & Rumors').classes('text-lg font-semibold text-slate-100 mb-2')
                            ui.label('Leave a note in the square. For now it is a local chronicle feed, but the panel is staged to grow into a broader town channel later.').classes('text-slate-400 text-sm leading-6')
                    ...
                    if state.game_tab == 'inventory':
                                ...
                                with ui.column().classes('flex-1 min-w-[320px] gap-4'):
                                    with ui.card().classes('mq-card w-full p-4 h-full').style('min-height: 620px;'):
                                        ...
                                        if not item_map:
                                            ...
                                            ui.label('No Fine-or-better same-tier transmutation pair is available in your inventory right now.').classes('mq-inv-empty mt-4')
                                        else:
                                            ...
                                            with ui.row().classes('w-full gap-3 mt-4 flex-wrap'):
                                                ui.select(
                                                    TRANSMUTE_TIER_SORT_OPTIONS,
                                                    value=state.transmute_sort_settings()[0],
                                                    label='Tier Sort',
                                                    on_change=lambda e: (setattr(state, 'transmute_tier_sort', e.value or 'Tier (High-Low)'), state.sync_transmute_selection(), request_render_refresh(force=True)),
                                                ).props('outlined options-dense popup-content-class=mq-item-select-menu options-selected-class=mq-item-select-option-selected behavior=menu').classes('w-[220px] mq-item-select')
                                                ui.select(
                                                    TRANSMUTE_RARITY_SORT_OPTIONS,
                                                    value=state.transmute_sort_settings()[1],
                                                    label='Rarity Sort',
                                                    on_change=lambda e: (setattr(state, 'transmute_rarity_sort', e.value or 'Rarity (High-Low)'), state.sync_transmute_selection(), request_render_refresh(force=True)),
                                                ).props('outlined options-dense popup-content-class=mq-item-select-menu options-selected-class=mq-item-select-option-selected behavior=menu').classes('w-[220px] mq-item-select')
                                        ...
                                        if reveal_stage >= 3:
                                            with ui.row().classes('gap-2 mt-2 flex-wrap'):
                                                ...
                                                if category is not None:
                                                    ui.label(f'Set {SAVED_ITEM_SET_LABELS[category]}').classes('mq-inv-pill set')
                                        if revealed_lines:
                                            ui.label('\n'.join(revealed_lines)).classes('mq-detail-text mq-transmute-reveal-lines mt-3')
                                        elif state.transmute_reveal_active:
                                ...
                                with ui.column().classes('flex-1 min-w-[380px] gap-4'):
                                    ...
                                    with ui.card().classes('mq-card w-full p-4'):
                                        ...
                                        lines = [
                                            ...
                                            'Reward: 50% chance for an item drop matching the offering tier and type',
                        ...
                        with ui.column().classes('w-full gap-4'):
                            with ui.card().classes('mq-card mq-glossary-hero w-full p-6 md:p-7'):
                                ...
                                ui.label('The brass-bound ledger of MasterQuest, refreshed for the current build: drop rules, affix shaping, market services, proficiency, and risk routes in one polished place.').classes('mq-glossary-body leading-7 mt-2 max-w-[980px]')
