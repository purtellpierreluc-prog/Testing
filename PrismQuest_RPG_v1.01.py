...
.mq-title-kicker {
  ...
  padding: 10px 16px;
  border-radius: 999px;
  border: 1px solid rgba(241, 206, 133, 0.20);
  background: linear-gradient(180deg, rgba(38, 28, 16, 0.82), rgba(16, 14, 14, 0.76));
  color: #f4dfb1;
  font-size: 0.98rem;
  font-weight: 800;
...
def build_masterquest_scroll_window_html(line: str, message: str, status_text: str, route_label: str = '') -> str:
    entries: List[Tuple[str, str]] = []
    clean_line = str(line or '').strip()
    clean_message = str(message or '').strip()
    clean_route = str(route_label or '').strip()
    if clean_line:
        entries.append(('Oracle', clean_line))
    if clean_message:
        entries.append(('Prism', clean_message))
    for status_line in [part.strip() for part in str(status_text or '').splitlines() if part.strip()][:2]:
        entries.append(('Rite', status_line))
    if clean_route:
        entries.append(('Path', clean_route))
    if not entries:
        entries.append(('Oracle', 'The prism chamber waits in absolute silence.'))
    if len(entries) == 1:
        entries = entries * 3
    scroll_entries = entries + entries
    entry_html = ''.join(
        f"<div class='mq-masterquest-scroll-entry'><span class='mq-masterquest-scroll-label'>{html.escape(label)}</span><div class='mq-masterquest-scroll-text'>{html.escape(body)}</div></div>"
        for label, body in scroll_entries
    )
    return f"<div class='mq-masterquest-scroll-window'><div class='mq-masterquest-scroll-track'>{entry_html}</div></div>"
                ...
                with ui.row().classes('mq-shell w-full items-stretch gap-6 no-wrap max-[1200px]:flex-wrap'):
                    with ui.card().classes('mq-title-card w-full flex-1 p-4 lg:p-5'):
                        with ui.element('div').classes('mq-title-stage w-full'):
                            ui.html("<div class='mq-title-kicker'>THE CHRONICLE LEDGER</div><div class='mq-title-stage-runes'></div>")
                    ...
                    if state.game_tab == 'inventory':
                        ...
                        with ui.column().classes('w-full gap-4'):
                            with ui.card().classes('mq-card mq-masterquest-hero w-full p-5 md:p-6'):
                                ...
                                ui.label('Release the essence of light from the black prism. Match each Light Essence to its one true vessel. One wrong pairing ends the rite instantly.').classes('mq-masterquest-note mt-2 max-w-[1050px]')
                                ...
                                with ui.row().classes('w-full items-start gap-4 mt-4 max-[1180px]:flex-wrap'):
                                    ...
                                    with ui.card().classes('mq-panel-frame flex-[1.15] min-w-[360px] p-4'):
                                        ...
                                        ui.html(build_masterquest_scroll_window_html(state.current_masterquest_line, state.masterquest_message, status_text, route_label)).classes('mt-3 w-full')
