...
class Player(Fighter):
    ...
    def recalculate_stats(self) -> None:
        ...
        if weapon is not None:
            ...
            weapon_stat_factor = max(0.0, weighted_stat / 100.0)
        ...
        if charm is not None:
            ...
            spell_stat_factor = max(0.0, self.intelligence / 25.0)
...
def roll_value_for_stat(stat_key: str, item_level: int, roll_mode: str = 'normal') -> float:
    ...
    if roll_mode not in {'advantage', 'disadvantage'}:
        return first
    second = _single_roll_value_for_stat(stat_key, item_level)
    ...
    def refresh_public_ladder(self) -> bool:
        ...
        try:
            ...
            if self.public_ladder_rows_cache:
                self.public_ladder_status = f"{len(self.public_ladder_rows_cache)} contenders are etched into the live registrar ledger."
            else:
                self.public_ladder_status = 'The live ladder is empty. Be the first soul to etch a chronicle into it.'
            return True
        except Exception as exc:
            self.public_ladder_rows_cache = []
...
def reset_masterquest_scene_state(self) -> None:
    ...
    self.masterquest_essence_visuals = {key: MASTERQUEST_ESSENCE_BLUE_DATA_URI for key in MASTERQUEST_ESSENCE_KEYS}
...
def generate_well_item_drop(monster_level: int, player_class: str, magic_find: float) -> Item:
    available_buckets = get_available_drop_buckets(monster_level)
    item_level = random.choice(available_buckets or [1])
    rarity = shift_rarity(choose_rarity_with_magic_find(magic_find), 1)
    slot = random.choice(['weapon', 'armor', 'charm'])
    subtype = random.choice(ITEM_SUBTYPES[slot])
